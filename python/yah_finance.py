...
codes = ['0001.KL', 'ZIGA.BK', 'ZINC.JK', 'ZKC.SI', 'ZONE.JK', 'ZYRX.JK']  # Replace with actual codes
...
combined_historical = pd.concat([data['historical'] for data in data_dict.values() if not data['historical'].empty])
combined_sharesout = pd.concat([data['sharesout'] for data in data_dict.values() if not data['sharesout'].empty])
combined_dividends = pd.concat([data['dividends'] for data in data_dict.values() if not data['dividends'].empty])
...
combined_historical['date'] = pd.to_datetime(combined_historical['Date'], errors='coerce').dt.tz_localize(None)
...
def safe_parse_json(json_text):
    ...
    try:
        ...
        json_text_fixed = re.sub(r'([{,])\s*\'([^\']+)\'\s*:', r'\1"\2":', json_text_fixed)

        # Fix values with single quotes
        json_text_fixed = re.sub(r':\s*\'([^\']+)\'\s*([,}])', r': "\1"\2', json_text_fixed)

        # Handle remaining single quotes (general case)
        json_text_fixed = re.sub(r"(?<![a-zA-Z])'([^']*)'(?![a-zA-Z])", r'"\1"', json_text_fixed)
...
data_info_expanded = pd.concat([data_info.drop(columns=['companyOfficers', 'parsed_json']), expanded_json], axis=1)

...
# CATln_Border(paste('YOUR PC IS =', pMyPC))

# ==================================================================================================
# FUNCTION MAIN START 2023-10-05 14:33
# ==================================================================================================
...
# ==================================================================================================
DOWNLOAD_WSJ_STK_LIST = function(pCountry = 'INDONESIA', Save_Folder = paste0(CCPRData, 'IFRC_CCPR/'), Save_File = 'DOWNLOAD_WSJ_STK_LIST.rds') {
  # ------------------------------------------------------------------------------------------------
  To_Continu = T
  Nr         = 0
  Data_All   = data.table()
  ...
  while (To_Continu)
  {
    # Nr   = 4 
    Nr   = Nr + 1
    pURL = paste0('https://www.wsj.com/market-data/quotes/company-list/country/', tolower(pCountry), '/', Nr)
    CATln_Border(pURL)
    # xweb = IFRC_CCPR_WEB_SAVE_AS(pURL, 'c:/temp/stock.txt', ToDeleteTemp = F)
    content    = try(rvest::read_html(pURL))
    
    if (all(class(content)!='try-error'))
    {
      tables     = content %>% html_table(fill = TRUE)
      xData      = CHECK_CLASS(try(as.data.table(tables[[1]])))
      if (nrow(xData)>0)
      {
        xData   = CLEAN_COLNAMES(xData)
        Final_Data  = xData[, .(country=pCountry, symbol=word(word(name,2,2, '\\('),1,1,'\\)'), company_name=word(name,1,1, '\\('), exchange, supersector)]
        # Final_Data  = xData[, .(company_name=word(name,1,1, '\\('))]
        My.Kable.TB(Final_Data)
        Data_All = rbind(Data_All, Final_Data, fill=T)
      } else { To_Continu = F}
    }
  }
  Data_All[, ':='(codesource=symbol, code=symbol, date=Sys.Date())]
  My.Kable.TB(Data_All)
  Data_History = CCPR_READRDS(Save_Folder, Save_File, ToRestore = T)
  
  Data_History = unique(rbind(Data_History, Data_All, fill=T), by=c('country', 'symbol'))
  CATln_Border('HISTORY')
  My.Kable.TB(Data_History)
  
  CCPR_SAVERDS(Data_History, Save_Folder, Save_File, ToKable = T, SaveOneDrive = T)
  CATln_Border(paste('DOWNLOAD_WSJ_STK_LIST : ', pCountry, '- DONE.'))
}

# ==================================================================================================
DOWNLOAD_WSJ_STK_GENDER_BY_URL = function(pURL = 'https://www.wsj.com/market-data/quotes/ID/XIDX/BUKK/company-people/executive-profile/267274') {
  # ------------------------------------------------------------------------------------------------
  ...
  if (grepl('data-module-zone="executive_profile"', xweb))
  {
    xBody = str.extract(xweb, 'data-module-zone="executive_profile"', '</div> </div> </div>')
    people_name        = str.extract(xBody, '<h3>', '</h3>') ; CATln(people_name)
    people_position    = str.extract(xBody, '<h4>', '</h4>') ; CATln(people_position)
    people_description = str.extract(xBody, '<p class="txtBody">', '</p>') ; CATln(people_description)
    people_gender      = as.character(NA)
    if (grepl('Mr.| He', people_description)) { people_gender='MALE'}
    if (!is.na(people_gender) & (grepl('Mrs.|Miss| She', people_description))) { people_gender='FEMALE'}
    Final_Data = data.table(people_gender, people_name, people_position, people_description, url=pURL)
    My.Kable.All(Final_Data[, -c('url', 'people_description')])
  }
  return(Final_Data)
}

# ==================================================================================================
IFRC_CCPR_DOWNLOAD_WSJ_BOARD_BY_ISO2 = function (pFolder = paste0(CCPRData, 'LIST/') , ISO2 = 'MY', Nb_Group = 20, Nb_Min = 50) {
  # --------------------------------------------------------------------------------------------------
  # pFolder = paste0(CCPRData, 'LIST/')
  pFile   = 'STOCK_EXCHANGE_LIST.xlsx'
  List_Codes = setDT (read.xlsx(paste0(pFolder,pFile)))
  List_Codes = CLEAN_COLNAMES(List_Codes) [iso2 == ISO2]
  # List_Codes = ins_ref[code]
  # ins_ref[type == 'STK'& iso2 == 'ID', 1:3]
  List_Codes[, codesource := symbol]
  # List_Codes = List_Codes [iso2 == iso2]
  FileName = paste0('IFRC_CCPR_DOWNLOAD_WJS_',ISO2,'_BOARD')
  FilePath = paste0(CCPRData,'IFRC_CCPR/', FileName, '.rds')
  My.Kable.TB(List_Codes[, .(symbol, iso2, name, codesource)])
  
  if (file.exists(FilePath))
  {
    Data_old = CCPR_READRDS(paste0(CCPRData,'IFRC_CCPR/'), paste0(FileName, '.rds'), ToKable = T) 
    if (nrow(Data_old)>0) {
      Data_old        = Data_old[!is.na(codesource)]
      Data_old[, symbol:=word(codesource,2,2,'/')]
      List_Codes_ToDo = List_Codes[!symbol %in% Data_old$symbol ]
  ...
  My.Kable.TB(List_Codes_ToDo[, .(symbol, iso2, name)])
  
  if (nrow(List_Codes_ToDo)>0)
  {
    List_Codes_ToDo = List_Codes_ToDo[order(symbol)]
    ...
    for (i in 1:Nb_TODO)
    {
      ...
      iso2     = FileData[i]$iso2
      CATln("")
      CATln_Border(paste('RUN_LOOP_STK_WSJ : ',ISO2,  '=', i, paste0('(', Nb_Group,')'), '/', Nb_TODO, '/', Nb_Total, ' = ', pCode))
      Sys.sleep(1)
      My_Data  = data.table()
      WSJ_CODE = paste0 (ISO2,'/',pCode)
      # WSJ_CODE = paste0 ('SG','/','500')
      My_Data  = CHECK_CLASS(try(IFRC_CCPR_DOWNLOAD_WSJ_STK_PEOPLE (WSJ_CODE = WSJ_CODE)))
      
      if (nrow(My_Data)>0)
      {
        Data_List[[i]] = My_Data
      } else {
        Data_List[[i]] = data.table(date=Sys.Date(), codesource=WSJ_CODE, source='WSJ')
      }
      Sys.sleep(2)
      ...
      if (i %% Nb_Group == 0 | i == Nb_TODO)
      {
        ...
        {
          Data_final = unique(Data_final, by=c('codesource', 'member', 'people_name', 'position'))
          Data_final[, code := codesource]
          CCPR_SAVERDS(Data_final,paste0(CCPRData,'IFRC_CCPR/'), FileToSave)
        }
        # My.Kable(Data_final)
        End.Time     = Sys.time()
...
# ==================================================================================================
IFRC_CCPR_WEB_SAVE_AS = function(pURL, File_Temp, pSleep=1, ToDeleteTemp=T) {
  # ------------------------------------------------------------------------------------------------
  MyExecution_Pattern = 'py r:/python/savepage.py "URL" "FILE"'
...
# ==================================================================================================
IFRC_CCPR_GET_MYPC = function(ToPrint=F) {
  # ------------------------------------------------------------------------------------------------
  MYPC = ""
...
# ==================================================================================================
IFRC_CCPR_GET_MYPC_POWER = function(ToPrint=F) {
  # ------------------------------------------------------------------------------------------------
  MYPC_POWER = 3
  MyPC = toupper(IFRC_CCPR_GET_MYPC())
  if (file.exists('r:/r/my_pc_power.txt'))
  {
    PC_POWER = as.data.table(fread('r:/r/my_pc_power.txt'))
    MYPC_POWER_Dt = PC_POWER[mypc==MyPC]
    if (nrow(MYPC_POWER_Dt)==1)
    {
      MYPC_POWER = MYPC_POWER_Dt$power
    } else { MYPC_POWER = 3}
  }
  if (ToPrint) { print(MYPC_POWER)}
  return(MYPC_POWER)
}

IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_COUNTRY = function(File_Folder=CCPRData, ISO2 = 'VN', Nb_Group = 50, Nb_Min= 20) { 
  # ------------------------------------------------------------------------------------------------
  RELOAD_INSREF()
  List_Codes = ins_ref[ type=='STK' & !is.na(yah)][order(-capiusd)]
  List_Codes = List_Codes [iso2 == ISO2][, .(country, code, codesource=yah, name, continent)]
  continent  = unique(List_Codes$continent)
  
  FileName       = paste0('CCPR_DOWNLOAD_YAH_STK_CAPIUSD_CAPIUSD_', ISO2,'_TODAY')
  FilePath       = paste0(File_Folder,  FileName, '.rds')
  ...
  if (nrow(List_Codes)>0)
  {
    if (file.exists(FilePath))
    {
      Data_old = CHECK_CLASS(try(CCPR_READRDS( paste0(File_Folder), paste0(FileName, '.rds'), ToKable=T, ToRestore = T)))
      if (nrow(Data_old)>0) 
      {
        Data_old = Data_old[!is.na(codesource)]
      ...
      CATln(paste('FILE NO EXIST :', FilePath))
      Data_old = CHECK_CLASS(try(CCPR_READRDS( paste0(File_Folder), paste0('CCPR_DOWNLOAD_YAH_STK_CAPIUSD_CAPIUSD_', continent,'_TODAY', '.rds'), ToKable=T, ToRestore = T))) 
      if (nrow(Data_old)>0) 
      {
        Data_old = Data_old[!is.na(codesource)]
        List_Codes_ToDo = List_Codes[!codesource %in% Data_old$codesource]
    ...
    if (nrow(List_Codes_ToDo)>0)
    {
      ...
      for (i in 1:Nb_TODO)
      {
        ...
        CATln_Border(paste('DEV_IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_COUNTRY : ',  ISO2, '=', i, 
                           paste0('(', Nb_Group,')'), '/', Nb_TODO, '/', Nb_Total, ' = ', pCode))
        
        # pCode = 'AAPL'
        # pCode = 'ADHI.JK'
        
        My_Data = CHECK_CLASS(try(DOWNLOAD_YAH_STK_CAPIUSD_BY_CODE(pCodesource=pCode, EchoOn=F, ToKable=T)))
        
        # My_Data = CHECK_CLASS(try(IFRC_CCPR_DOWNLOAD_YAH_ESG_BY_CODE(pCode = pCode)))
        
        if (nrow(My_Data)>0)        {          Data_List[[i]] = My_Data        }
        Sys.sleep(2)
        ...
        if (i %% Nb_Group ==0 | i == Nb_TODO)
        {
          ...
          if (nrow(Data_final)>0)
          {
            Data_final = MERGE_DATASRC_CODE_BYCODESOURCE('YAH', Data_final)
            Data_final = MERGE_DATASTD_BYCODE(Data_final, pOption='FINAL')
            Data_final = unique(Data_final, by=c('code', 'date'))
            CCPR_SAVERDS(Data_final, FileFolder = File_Folder, paste0(FileName,'.rds'), SaveOneDrive = T, ToSummary = T, ToKable = T)
            # saveRDS(Data_final, paste0(CCPRData,FileName,'.rds' ) )
        ...
        # End_Forecast = Avg_Time * (Nb_TODO - i)
        # Time_To_End = as.difftime(End_Forecast, units = "secs") + Sys.time()
        CATln_Border(paste('DURATION :', Format.Number(difftime(End.Time, Start.Time, units='secs'),0)))
      }
      My.Kable.Min(Data_final)
    } else {
      CATln_Border('DATA FULLY UPDATED')
      IFRC_SLEEP(2)
      Data_final = Data_old
...
# ==================================================================================================
CCPR_UPLOAD_IFRC_CCPR_SAMPLE = function () { 
  # ------------------------------------------------------------------------------------------------
x = setDT(read.xlsx( 'R:/CCPR/DATA/FOR_WEB/ifrc_ccpr_sample.xlsx'))
# str(x)

list_report = x$file_name
for (i in 1: length (list_report))
{
  # i = 5
  filename = list_report [i]
  if (filename == 'ccpr_report_detail_files.rds') {data = CCPR_READRDS('R:/CCPR/DATA/', filename, ToRestore = T, ToKable = T)} else {
  data = CCPR_READRDS('R:/CCPR/DATA/FOR_WEB/', filename, ToRestore = T) }
  x$records [i] = nrow(data)
}
x$upload = substr(Sys.time(),1,16)
saveRDS(x, 'R:/CCPR/DATA/FOR_WEB/ifrc_ccpr_sample.rds')
CCPR_FTP_UPLOAD_FOLDER_FILE(File_Folder='R:\\\\CCPR\\\\DATA\\\\FOR_WEB\\\\', File_Name=tolower('ifrc_ccpr_sample.rds'), File_To=tolower('ifrc_ccpr_sample.rds'))
}


# ==================================================================================================
RUN_NOSTOP_BY_PC_CCPR_ESG = function(pMyPC){
  # ------------------------------------------------------------------------------------------------
  List_Sector = list(
  ...
  switch(uMyPC,
         'BEQ-RD1' = {  
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'EUROPE', Nb_Group = 50, Nb_Min = 500))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'EUROPE', Nb_Group = 50, Nb_Min = 500))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'EUROPE', Nb_Group = 50, Nb_Min = 500))
         },
         
         'BEQ-RD2' = {  
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'AFRICA', Nb_Group = 50, Nb_Min = 500))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'EUROPE', Nb_Group = 50, Nb_Min = 500))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'AFRICA', Nb_Group = 50, Nb_Min = 500))
         },
         
         'BEQ-RD3' = {  
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'ASIA', Nb_Group = 50, Nb_Min = 500))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'AMERICA', Nb_Group = 50, Nb_Min = 500))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'ASIA', Nb_Group = 50, Nb_Min = 500))
         },
         
         'BEQ-RD4' = {  
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'OCEANIA', Nb_Group = 50, Nb_Min = 500))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'ASIA', Nb_Group = 50, Nb_Min = 500))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'OCEANIA', Nb_Group = 50, Nb_Min = 500))
         },
         
         'BEQ-RD5' = {  
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'AMERICA', Nb_Group = 50, Nb_Min = 500))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'AMERICA', Nb_Group = 50, Nb_Min = 500))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'AMERICA', Nb_Group = 50, Nb_Min = 500))
         }
  )
  # try(CHECK_FILE_QUALITY_IFRC())
  # INSERT 
...
# ==================================================================================================
RUN_NOSTOP_BY_PC_CCPR_CAPIUSD = function(pMyPC){
  # ------------------------------------------------------------------------------------------------
  List_Sector = list(
    c('BSM', 'Basic Materials', 'https://finance.yahoo.com/screener/predefined/ms_basic_materials/'),
    c('CMS', 'Communication Services', 'https://finance.yahoo.com/screener/predefined/ms_communication_services/'),
  ...
  switch(uMyPC,
         'BEQ-RD1' = {  
           ...
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'EUROPE', Nb_Group = 50, Nb_Min = 500))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'EUROPE', Nb_Group = 50, Nb_Min = 500))
           x = try( IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_COUNTRY(File_Folder=CCPRData, ISO2 = 'VN', Nb_Group = 50, Nb_Min= 20) )
           x = try( IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_COUNTRY(File_Folder=CCPRData, ISO2 = 'JP', Nb_Group = 50, Nb_Min= 20) )
         },
         
         'BEQ-RD2' = {  
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'AFRICA', Nb_Group = 50, Nb_Min = 500))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'AFRICA', Nb_Group = 50, Nb_Min = 500))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'EUROPE', Nb_Group = 50, Nb_Min = 500))
           x = try( IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_COUNTRY(File_Folder=CCPRData, ISO2 = 'HK', Nb_Group = 50, Nb_Min= 20) )
           x = try( IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_COUNTRY(File_Folder=CCPRData, ISO2 = 'SG', Nb_Group = 50, Nb_Min= 20) )
         },
         ...
         'BEQ-RD4' = {  
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'OCEANIA', Nb_Group = 50, Nb_Min = 500))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'OCEANIA', Nb_Group = 50, Nb_Min = 500))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'AMERICA', Nb_Group = 50, Nb_Min = 500))
           x = try( IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_COUNTRY(File_Folder=CCPRData, ISO2 = 'CN', Nb_Group = 50, Nb_Min= 20) )
         },
         ...
         'BEQ-RD5' = {  
           ...
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'AMERICA', Nb_Group = 50, Nb_Min = 500))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'ASIA', Nb_Group = 50, Nb_Min = 500))
           x = try( IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_COUNTRY(File_Folder=CCPRData, ISO2 = 'KR', Nb_Group = 50, Nb_Min= 20) )
           # x = try( IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_COUNTRY(File_Folder=CCPRData, ISO2 = 'VN', Nb_Group = 50, Nb_Min= 20) )
         }
  )
  # try(CHECK_FILE_QUALITY_IFRC())
...
# ==================================================================================================
CCPR_DATASET_FOR_WEB = function(){
  # ------------------------------------------------------------------------------------------------
  x = setDT( read.xlsx('R:/CCPR/DATA/FOR_WEB/FOR_WEB_DATASET_URL.xlsx'))
  x$UPLOAD = substr(Sys.time(),1,16)
  saveRDS(x, 'R:/CCPR/DATA/FOR_WEB/for_web_dataset_url.rds')
  
  CCPR_FTP_UPLOAD_FOLDER_FILE(File_Folder='R:\\\\CCPR\\\\DATA\\\\FOR_WEB\\\\', File_Name=tolower('for_web_dataset_url.rds'), File_To=tolower('for_web_dataset_url.rds'))
}

# ==================================================================================================
CCPR_REPORT_FOR_WEB = function ( pFolder = 'R:/CCPR/DATA/FOR_WEB/', pFile = 'ccpr_stkvn_report_info.rds' ,FileUpLoad ='ccpr_stkvn_report.rds'  ) {
  # ------------------------------------------------------------------------------------------------
  y = CCPR_READRDS(pFolder,pFile, ToKable = T) [order(file)] [!is.na(report_name)]
  upload =  substr( Sys.time(), 1, 16)
  final = y[,.(file = report_name , description = NA, url = NA,pc_not_na,start,date,codes,records, hsx, hnx, upc,updated , upload)]
  # write.xlsx(final, 'R:/CCPR/DATA/FOR_WEB/CCPR_STKVN_REPORT.xlsx')
  # final = x
  for (i in 1: nrow(final))
  {
    # i = 1
    file = final [i]$file
    dataset = word(file,1,1,'_')
    period  = word(file,2,2,'_')
    filename = tolower(paste0('ccpr_stkvn_',dataset,'_', period, '.rds'))
    # FolderRDS = gsub('/','\\\\\\\\', pFolder)
    # k = try(CCPR_READRDS('R:/CCPR/DATA/FOR_WEB/', filename))
    CATln_Border(paste0(pFolder, filename))
    CCPR_FTP_UPLOAD_FOLDER_FILE(File_Folder='R:\\\\CCPR\\\\DATA\\\\FOR_WEB\\\\', File_Name=tolower(filename), File_To=tolower(filename))
    Sys.sleep (2)
    k = paste0('https://datacenter.ccpr.vn/api/get_file/demo@ccpr.vn:Demo2023?filerds=',filename,'&fields=code,date&label=CODE,DATE&width10,10&format=c,d&title=',gsub(' ','%20',file),'&note=Source:%20CCPR')
    # Sys.sleep (30)
    final$url[i] = k
  }
  
  saveRDS(final, paste0(pFolder,FileUpLoad))
  CCPR_FTP_UPLOAD_FOLDER_FILE(File_Folder='R:\\\\CCPR\\\\DATA\\\\FOR_WEB\\\\', File_Name=tolower(FileUpLoad), File_To=tolower(FileUpLoad))
  # CREATE_JQGRID_API_URL(pFolder=pFolder, FileName = tolower(FileUpLoad))
}

# ==================================================================================================
IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT = function(File_Folder=CCPRData, Continent = 'ASIA', Nb_Group = 50, Nb_Min= 200) { 
  # ------------------------------------------------------------------------------------------------
  RELOAD_INSREF()
  List_Codes = ins_ref[ type=='STK' & !is.na(yah)][order(-capiusd)]
  List_Codes = List_Codes [continent == Continent][, .(country, code, codesource=yah, name)]
  
  FileName       = paste0('CCPR_DOWNLOAD_YAH_STK_ESG_CAPIUSD_', Continent,'_TODAY')
  FilePath       = paste0(File_Folder,  FileName, '.rds')
  CATln_Border(paste("SAVE TO FILEPATH : ", FilePath))
  ...
  if (nrow(List_Codes)>0)
  {
    ...
    if (nrow(List_Codes_ToDo)>0)
    {
      ...
      for (i in 1:Nb_TODO)
      {
        ...
        CATln("")
        # CATln_Border(paste(i, '/', Nb_TODO, ' = ', pCode))
        CATln_Border(paste('DEV_IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT : ',  Continent, '=', i, 
                           paste0('(', Nb_Group,')'), '/', Nb_TODO, '/', Nb_Total, ' = ', pCode))
...
# ==================================================================================================
IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT = function(File_Folder=CCPRData, Continent = 'ASIA', Nb_Group = 50, Nb_Min= 200) { 
  # ------------------------------------------------------------------------------------------------
  RELOAD_INSREF()
  List_Codes = ins_ref[ type=='STK' & !is.na(yah)][order(-capiusd)]
  List_Codes = List_Codes [continent == Continent][, .(country, code, codesource=yah, name)]
  
  FileName       = paste0('CCPR_DOWNLOAD_YAH_STK_CAPIUSD_CAPIUSD_', Continent,'_TODAY')
  FilePath       = paste0(File_Folder,  FileName, '.rds')
  CATln_Border(paste("SAVE TO FILEPATH : ", FilePath))
  ...
  if (nrow(List_Codes)>0)
  {
    ...
    if (nrow(List_Codes_ToDo)>0)
    {
      ...
      for (i in 1:Nb_TODO)
      {
        ...
        CATln("")
        # CATln_Border(paste(i, '/', Nb_TODO, ' = ', pCode))
        CATln_Border(paste('DEV_IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT : ',  Continent, '=', i, 
                           paste0('(', Nb_Group,')'), '/', Nb_TODO, '/', Nb_Total, ' = ', pCode))
...
# ==================================================================================================
RUN_NOSTOP_BY_PC_CCPR_GENDER = function(pMyPC){
  # ------------------------------------------------------------------------------------------------
  List_Sector = list(
    c('BSM', 'Basic Materials', 'https://finance.yahoo.com/screener/predefined/ms_basic_materials/'),
    c('CMS', 'Communication Services', 'https://finance.yahoo.com/screener/predefined/ms_communication_services/'),
  ...
  switch(uMyPC,
         'BEQ-RD1' = {  
           x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'EUROPE', Nb_Group = 50, Nb_Min = 5000))
           # x = try( IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_COUNTRY(File_Folder=CCPRData, ISO2 = 'VN', Nb_Group = 50, Nb_Min= 20) )
         ...
         'BEQ-RD2' = {  
           x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'AFRICA', Nb_Group = 50, Nb_Min = 5000))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'AFRICA', Nb_Group = 50, Nb_Min = 500))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'AFRICA', Nb_Group = 50, Nb_Min = 500))
         },
         
         'BEQ-RD3' = {  
           x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'OCEANIA', Nb_Group = 50, Nb_Min = 5000))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'ASIA', Nb_Group = 50, Nb_Min = 500))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'ASIA', Nb_Group = 50, Nb_Min = 500))
         },
         
         'BEQ-RD4' = {  
           x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'AMERICA', Nb_Group = 50, Nb_Min = 5000))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'OCEANIA', Nb_Group = 50, Nb_Min = 5000))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'EUROPE', Nb_Group = 50, Nb_Min = 5000))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'AFRICA', Nb_Group = 50, Nb_Min = 5000))
           x = try(IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,   Continent = 'ASIA', Nb_Group = 50, Nb_Min = 5000))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_ESG_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,      Continent = 'OCEANIA', Nb_Group = 50, Nb_Min = 500))
           # x = try(IFRC_CCPR_DOWNLOAD_YAH_CAPIUSD_CAPIUSD_BY_CONTINENT(File_Folder=CCPRData,  Continent = 'OCEANIA', Nb_Group = 50, Nb_Min = 500))
         },
...
# ==================================================================================================
IFRC_CCPR_DOWNLOAD_YAH_GENDER_BY_CONTINENT = function(Nb_Group = 50, Nb_Min= 200, AddParam='EUROPE') { 
  # ------------------------------------------------------------------------------------------------
  
  x = ins_ref [yah_mega == 1]
  # My.Kable.TB(x)
  list_code = x[,.(iso2,country,continent,code, name,codesource = yah,isin,link)]
  List_Companies = list_code [continent == AddParam]
  List_Codes     = unique(List_Companies[,.(codesource, name)], by ='codesource')
  My.Kable(List_Codes)
  FileName       = paste0('IFRC_CCPR_DOWNLOAD_YAH_MEGA_BOARD_CONTINENT', AddParam,'_TODAY')
  ...
  if (nrow(List_Codes)>0)
  {
    if (file.exists(FilePath))
    {
      Data_old = CCPR_READRDS(paste0(CCPRData),paste0(FileName, '.rds'))
      if (nrow(Data_old)>0) {
        Data_old = Data_old[!is.na(codesource)]
        List_Codes_ToDo = List_Codes[!codesource %in% Data_old$codesource]
        My.Kable.MaxCols(Data_old)
    ...
    if (nrow(List_Codes_ToDo)>0)
    {
      ...
      for (i in 1:Nb_TODO)
      {
        ...
        CATln("")
        # CATln_Border(paste(i, '/', Nb_TODO, ' = ', pCode))
        CATln_Border(paste('IFRC_CCPR_DOWNLOAD_YAH_MEGA_BOARD_CONTINENT : ', pOption, AddParam, '=', i, paste0('(', Nb_Group,')'), '/', Nb_TODO, '/', Nb_Total, ' = ', pCode))
        
        My_Data = CHECK_CLASS(try(IFRC_CCPR_DOWNLOAD_YAH_STK_BOARD(pCode = pCode)))
        ...
        if (i %% Nb_Group ==0 | i == Nb_TODO)
        {
          ...
          CCPR_SAVERDS(Data_final, FileFolder = CCPRData, paste0(FileName,'.rds'), SaveOneDrive = T, ToSummary = T)
          saveRDS(Data_final, paste0(CCPRData,FileName,'.rds' ) )
          CATln(paste('SAVED : ', FileToSave, '-', Format.Number(nrow(Data_final),0),'records'))
          Sys.sleep(2)
        }
...
# ==================================================================================================
IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT = function(File_Folder=CCPRData, Continent = 'ASIA', Nb_Group = 50, Nb_Min= 200) { 
  # ------------------------------------------------------------------------------------------------
  RELOAD_INSREF()
  List_Codes = ins_ref[ type=='STK' & !is.na(yah)][order(-capiusd)]
  List_Codes = List_Codes [continent == Continent][, .(country, code, codesource=yah, name)]
  
  FileName       = paste0('CCPR_DOWNLOAD_YAH_STK_GENDER_CAPIUSD_', Continent,'_TODAY')
  FilePath       = paste0(File_Folder,  FileName, '.rds')
  CATln_Border(paste("SAVE TO FILEPATH : ", FilePath))
  ...
  if (nrow(List_Codes)>0)
  {
    ...
    if (nrow(List_Codes_ToDo)>0)
    {
      ...
      for (i in 1:Nb_TODO)
      {
        ...
        CATln("")
        # CATln_Border(paste(i, '/', Nb_TODO, ' = ', pCode))
        CATln_Border(paste('DEV_IFRC_CCPR_DOWNLOAD_YAH_GENDER_CAPIUSD_BY_CONTINENT : ',  Continent, '=', i, 
                           paste0('(', Nb_Group,')'), '/', Nb_TODO, '/', Nb_Total, ' = ', pCode))
        ...
        if (i %% Nb_Group ==0 | i == Nb_TODO)
        {
          ...
          if (nrow(Data_final)>0)
          {
            Data_final = unique(Data_final, by=c('code', 'date', 'source_name'))
            CCPR_SAVERDS(Data_final, FileFolder = File_Folder, paste0(FileName,'.rds'), SaveOneDrive = T, ToSummary = T, ToKable = T)
            # saveRDS(Data_final, paste0(CCPRData,FileName,'.rds' ) )
            CATln(paste('SAVED : ', FileToSave, '-', Format.Number(nrow(Data_final),0),'records'))
            Sys.sleep(2)
...
# DEV_IFRC_CCPR_DOWNLOAD_YAH_GENDER_BY_CONTINENT (File_Folder=CCPRData, Continent = 'ASIA', Nb_Group = 50, Nb_Min= 200) 

# ==================================================================================================
RUN_NOSTOP_YAH_BY_PC = function(pMyPC){
  # ------------------------------------------------------------------------------------------------
  switch(pMyPC,
         'BeQ-RD1'  = {  
           try(DEV_IFRC_CCPR_DOWNLOAD_YAH_ESG_BY_CONTINENT (File_Folder=CCPRData, Continent = 'EUROPE', Nb_Group = 50, Nb_Min= 200))
           try(DEV_IFRC_CCPR_DOWNLOAD_YAH_GENDER_BY_CONTINENT (File_Folder=CCPRData, Continent = 'EUROPE', Nb_Group = 50, Nb_Min= 200))
         },
         'BeQ-RD2'  = {  
           try(DEV_IFRC_CCPR_DOWNLOAD_YAH_ESG_BY_CONTINENT (File_Folder=CCPRData, Continent = 'AFRICA', Nb_Group = 50, Nb_Min= 200))
           try(DEV_IFRC_CCPR_DOWNLOAD_YAH_GENDER_BY_CONTINENT (File_Folder=CCPRData, Continent = 'ASIA', Nb_Group = 50, Nb_Min= 200))
         },
         'BeQ-RD4'  = {  
           try(DEV_IFRC_CCPR_DOWNLOAD_YAH_ESG_BY_CONTINENT (File_Folder=CCPRData, Continent = 'OCEANIA', Nb_Group = 50, Nb_Min= 200))
           try(DEV_IFRC_CCPR_DOWNLOAD_YAH_GENDER_BY_CONTINENT (File_Folder=CCPRData, Continent = 'OCEANIA', Nb_Group = 50, Nb_Min= 200))
         },
         'BeQ-RD5'  = {  
           try(DEV_IFRC_CCPR_DOWNLOAD_YAH_ESG_BY_CONTINENT (File_Folder=CCPRData, Continent = 'ASIA', Nb_Group = 50, Nb_Min= 200))
           try(DEV_IFRC_CCPR_DOWNLOAD_YAH_GENDER_BY_CONTINENT (File_Folder=CCPRData, Continent = 'AFRICA', Nb_Group = 50, Nb_Min= 200))
         },
         'BeQ-RD3'  = {  
           try(DEV_IFRC_CCPR_DOWNLOAD_YAH_ESG_BY_CONTINENT (File_Folder=CCPRData, Continent = 'AMERICA', Nb_Group = 50, Nb_Min= 200))
           try(DEV_IFRC_CCPR_DOWNLOAD_YAH_GENDER_BY_CONTINENT (File_Folder=CCPRData, Continent = 'AMERICA', Nb_Group = 50, Nb_Min= 200))
         }
  )
}

# ==================================================================================================
IFRC_CCPR_DOWNLOAD_YAH_ESG_BY_CONTINENT = function(File_Folder=CCPRData, Continent = 'ASIA', Nb_Group = 50, Nb_Min= 200) { 
  # ------------------------------------------------------------------------------------------------
  RELOAD_INSREF()
  List_Codes = ins_ref[ type=='STK' & !is.na(yah) & yah_mega==1 ]
  List_Codes = List_Codes [continent == Continent][, .(country, code, codesource=yah, name)]
  
  FileName       = paste0('CCPR_DOWNLOAD_YAH_STK_ESG_MEGA_', Continent,'_TODAY')
  FilePath       = paste0(File_Folder,  FileName, '.rds')
  CATln_Border(paste("SAVE TO FILEPATH : ", FilePath))
  ...
  if (nrow(List_Codes)>0)
  {
    ...
    if (nrow(List_Codes_ToDo)>0)
    {
      ...
      for (i in 1:Nb_TODO)
      {
        ...
        CATln("")
        # CATln_Border(paste(i, '/', Nb_TODO, ' = ', pCode))
        CATln_Border(paste('IFRC_CCPR_DOWNLOAD_YAH_ESG_BY_CONTINENT : ',  Continent, '=', i, 
                           paste0('(', Nb_Group,')'), '/', Nb_TODO, '/', Nb_Total, ' = ', pCode))
...
# ==================================================================================================
IFRC_CCPR_DOWNLOAD_YAH_STK_BOARD = function(pCode = 'AAPL') {
  # ------------------------------------------------------------------------------------------------
  # x = IFRC_CCPR_DOWNLOAD_YAH_STK_BOARD(pCode = 'VNM.VN')
  # x = IFRC_CCPR_DOWNLOAD_YAH_STK_BOARD(pCode = 'TENA.KL')
  DATA_AVAILABLE = F
  pURL  = paste0('https://finance.yahoo.com/quote/', pCode, '/profile')
  CATln_Border(pURL)
  content    = try(rvest::read_html(pURL))
  if (all(class(content)!='try-error'))
  {
    ...
    if (length(tables)>1)
    {
      ...
      if ('Title' %in% names(xData))
      {
        DATA_AVAILABLE = T
        xData      = CLEAN_COLNAMES(xData)
        xData$cob  = as.character(NA)
        xData$ceo  = as.character(NA)
        xData[, name:=gsub('Ph.D.','', name)]
        xData[, title:=trimws(gsub('Sr. ','', title))]
        xData[, gtitle:=word(name,1,1,'[.]')]
        xData[toupper(gtitle)=='MR', gender:='MALE']
        xData[toupper(gtitle)=='MRS', gender:='FEMALE']
        xData[toupper(gtitle)=='MS', gender:='FEMALE']
        xData[, source_name:=trimws(word(name,2,2,'[.]'))]
        xData[, source_name:=gsub('Ph.D.','', source_name)]
        xData[, nr:=seq.int(1,.N)]
        xData[grepl('Chairman of Board|Chairman|Chairwoman', title), cob:=1]
        xData[grepl('^CEO', title), ceo:=1]
        xData[grepl('CEO$', title), ceo:=1]
        if (nrow(xData[ceo==1])==0) { xData[title=='Pres & Exec. Director' & nr==1, ceo:=1]}
        if (nrow(xData[ceo==1])==0) { xData[grepl('^Pres', title) & nr<=2, ceo:=1]}
        xData[yearborn=='N/A', yearborn:=as.character(NA)]
        xData[, yearborn:=as.numeric(yearborn)]
        # x = 'Huu Minh Mai'
        # word(x, - 1)    
        xData[, people_lastname:=toupper(word(source_name,-1))]
        # xData[, people_firstname:=trimws(gsub(paste0(people_lastname,'[$]'),'',source_name, ignore.case = T))]
        xData[, people_firstname:=trimws(substr(source_name, 1, nchar(source_name)-nchar(people_lastname)))]
        xData[, codesource:=pCode]
        LastTrading = SYSDATETIME(9)
        xData[, date:=LastTrading]
        xData = MERGE_DATASRC_CODE_BYCODESOURCE('YAH', xData)
        if (nrow(xData[!is.na(code)])>0) { xData = MERGE_DATASTD_BYCODE(xData, pOption='FINAL')}
        xData = xData %>% dplyr::select('nr', 'codesource', 'date', 'people_lastname', 'people_firstname', 'source_name', 'ceo', 'cob', 'gender', everything())
        My.Kable.All(xData[, 1:12])
      }
    } else { DATA_AVAILABLE = F }
  } 
  if (!DATA_AVAILABLE)
  {
    LastTrading = SYSDATETIME(9)
    xData = data.table(source='YAH', codesource=pCode, dataset='GENDER', date=LastTrading)
    xData = MERGE_DATASRC_CODE_BYCODESOURCE('YAH', xData)
    xData = MERGE_DATASTD_BYCODE(xData, pOption='FINAL')
    My.Kable.All(xData)
  }
  return(xData)
}
...
# ==================================================================================================
IFRC_CCPR_DOWNLOAD_YAH_ESG_BY_CODE = function(pCode = 'ZOMATO.NS')
{
  Final_Data = data.table()
  DATA_AVAILABLE = F
  
  pURL  = paste0('https://finance.yahoo.com/quote/',pCode, '/sustainability')
  x = try(readr::read_file(pURL))
  if (all(class(x)!='try-error'))
  {
    DATA_AVAILABLE = T
    if (grepl('<span>Total ESG Risk score</span>', x)) 
    { 
      xtemp = gsub('"','', str.extract(x, '<span>Total ESG Risk score</span>', '<span'))
      ESG_TOTAL = as.numeric(str.extract(word(xtemp, 3,3, '<div class='),'>', '<'))
    } else { ESG_TOTAL = as.numeric(NA)}
    
    if (grepl('<span>Environment Risk Score</span>', x)) 
    { 
      xtemp = gsub('"','', str.extract(x, '<span>Environment Risk Score</span>', '<span'))
      ESG_E = as.numeric(str.extract(word(xtemp, 3,3, '<div class='),'>', '<'))
    } else { ESG_E = as.numeric(NA)}
    
    if (grepl('<span>Social Risk Score</span>', x)) 
    { 
      xtemp = gsub('"','', str.extract(x, '<span>Social Risk Score</span>', '<span'))
      ESG_S = as.numeric(str.extract(word(xtemp, 3,3, '<div class='),'>', '<'))
    } else { ESG_S = as.numeric(NA)}
    
    if (grepl('<span>Governance Risk Score</span>', x)) 
    { 
      xtemp = gsub('"','', str.extract(x, '<span>Governance Risk Score</span>', '<span'))
      ESG_G = as.numeric(str.extract(word(xtemp, 3,3, '<div class='),'>', '<'))
    } else { ESG_G = as.numeric(NA)}
    Final_Data = data.table(codesource=pCode, source='YAH', dataset='ESG SCORE', esg_total=ESG_TOTAL, esg_e=ESG_E, esg_s=ESG_S, esg_g=ESG_G)
  ...
  {
    LastTrading = SYSDATETIME(9)
    Final_Data = data.table(source='YAH', codesource=pCode, dataset='ESG', date=LastTrading)
    Final_Data = MERGE_DATASRC_CODE_BYCODESOURCE('YAH', Final_Data)
    Final_Data = MERGE_DATASTD_BYCODE(Final_Data, pOption='FINAL')
    My.Kable.All(Final_Data)
  }
...
# ==================================================================================================
OLD_IFRC_CCPR_DOWNLOAD_YAH_ESG_BY_CODE = function(pCode = 'ZOMATO.NS')
{
  Final_Data = data.table()
  pURL  = paste0('https://finance.yahoo.com/quote/',pCode, '/sustainability')
  x = try(readr::read_file(pURL))
...
# ==================================================================================================
CCPR_DATASET_QUALITY = function(pData=data.table(), FileFolder=CCPRData, FileName='download_hsx_stkvn_ref_history.rds', pPref = 'sharesout') {
  #-------------------------------------------------------------------------------------------------
  ToContinu = F
  if (nrow(pData)==0)
  {
    ...
    {
      pFileName = FileName
      xData = CHECK_CLASS(try(CCPR_READRDS(FileFolder = FileFolder, FileName = FileName))) [!is.na(shareslis) ] 
    }
  } else { 
    xData = pData 
    pFileName = ''
  ...
  if (nrow(xData)>0)
  {
    if (pPref %in% names(xData)) 
    {
      Fields = c('code', 'date','source', pPref)
      xData_Field = xData[, ..Fields]
      colnames(xData_Field) = c('code', 'date','source', 'value')
      xData_Field[, dataset:=pPref]
      # My.Kable(xData_Field)
      xData_Summary = xData_Field[, .(codes=.N, dataset=dataset[1], source = xData_Field$source[1]), by='date'][order(date)]
      xData_Summary[, nr:=seq.int(1, .N)]
      xData_Summary[ order(-date), nrdesc:=seq.int(1, .N)]
      xData_Summary[, cumrecords:=cumsum(codes)]
      xData_Summary[, filename:=toupper(pFileName)]
      My.Kable.TB(xData_Summary)
    }
  }
  return(xData_Summary)
}

# ==================================================================================================
CCPR_REPORT_DETAIL_FOR_WEB = function( list = c('CCPR_DOWNLOAD_C68_STKVN_SHARES_HISTORY.RDS',
                                                'CCPR_DOWNLOAD_VST_STKVN_SHARES_HISTORY.RDS',
                                                'CCPR_DOWNLOAD_CAF_STKVN_SHARES_HISTORY.RDS', 
                                                'CCPR_DOWNLOAD_HSX_STKVN_SHARES_HISTORY.rds',          
                                                'CCPR_DOWNLOAD_STB_STKVN_SHARES_HISTORY.RDS') , pPref =  'shareslis') {
  # ------------------------------------------------------------------------------------------------
  # CCPR_FTP_UPLOAD_FOLDER_FILE(File_Folder='R:\\\\CCPR\\\\DATA\\\\FOR_WEB', File_Name=tolower(  'CCPR_DOWNLOAD_STB_STKVN_SHARES_HISTORY.RDS'), File_To=tolower(  'CCPR_DOWNLOAD_STB_STKVN_SHARES_HISTORY.RDS'))
  my_data = list()
  ccpr_stkvn_market_day <- CCPR_READRDS('R:/CCPR/DATA/FOR_WEB/','ifrc_ccpr_stkvn_market_day.rds', ToRestore = T)
  datax = unique(ccpr_stkvn_market_day[order(-date)], by = 'code')
  datax[, market := close]
  for ( i in 1:length(list))
  {
    # i = 5
    data = try(CCPR_DATASET_QUALITY(pData=data.table(), FileFolder=CCPRData, FileName=list[i], pPref = pPref) [!is.na (date)])
    Sys.sleep(2)

    
    dataset = data[nr == max(nr)]$dataset
    source  = data[nr == max(nr)]$source
    upload  = substr(Sys.time(),1,16)
    file = gsub('.RDS|.rds','',list[i])
    market = CCPR_READRDS(CCPRData, list[i], ToRestore = T)[!is.na (date)] [date == max(date)]
    if (!'sharesout' %in% colnames(market)) {market[, sharesout:= NA]}
    data_market = merge(market[,.(code,codesource,source,date,sharesout,shareslis,close)], datax[,.(code,market)], by = 'code')
    # if ('market' %in% colnames(data_market)) {
    data_market[,nr_market := seq(.N), by = 'market']
    if(max(data_market [market == 'HSX']$nr_market) > 0 ) { hsx = max(data_market [market == 'HSX']$nr_market)} else {hsx = NA }
    if(max(data_market [market == 'HNX']$nr_market) > 0 ) { hnx = max(data_market [market == 'HNX']$nr_market)} else {hnx = NA }
    if(max(data_market [market == 'UPC']$nr_market)  > 0 ) { upc = max(data_market [market == 'UPC']$nr_market) } else {upc = NA }
    # upc = max(data_market [market == 'UPC']$nr_market)  
    # } else { hsx = NA; hnx = NA; upc = NA}
    # description = NA
    data_all = CCPR_READRDS(CCPRData, list[i], ToRestore = T, ToKable = T) 
    data_all[!is.na(date), firstdate := min(date)]
    data_all$days = nrow(unique(data_all[!is.na(date)], by = 'date'))
    data_all[, upload := substr(Sys.time(),1,16)]
    CCPR_SAVERDS(data_all,CCPRData, list[i])
    CCPR_FTP_UPLOAD_FOLDER_FILE(File_Folder='R:\\\\CCPR\\\\DATA\\\\FOR_WEB', File_Name=tolower(list[i]), File_To=tolower(list[i]))
    Sys.sleep(2)
    max_date = max(data_all [!is.na(date)]$date)
    data_all = data_all[date == max_date]
    k = paste0('https://datacenter.ccpr.vn/api/get_file/demo@ccpr.vn:Demo2023?filerds=',tolower(list[i]),'&fields=source,codesource,ticker,code,firstdate,date,days,sharesout,shareslis,upload&label=SOURCE,CODESOURCE,TICKER,CODE,FIRSTDATE,DATE,DAYS,SHARESOUT,SHARESLIS,UPLOAD&width=8,8,8,10,10,10,5,12,12,15&format=c,c,c,c,d,d,n0,n0,n0,d')
    detail = paste0(k,'&title=',toupper(dataset),'%20',toupper(source),'%20HISTORY&note=Source:%20CCPR')
    Sys.sleep(2)
    
    pc_not_na = round(nrow(data_all[!is.na(shareslis)])/nrow(data_all)*100,2)
    my_data [[i]] = data.table (dataset = toupper(dataset),source = source, file = file ,description = detail ,date = data[nr == max(nr)]$date ,date_1 = data[nrdesc == 2]$date,
                                codes = data[nr == max(nr)]$codes, pc_not_na = pc_not_na ,days = max(data$nr),
                                records = data[nr == max(nr)]$cumrecords,hsx = hsx, hnx=hnx, upc=upc, upload = upload)
  }
  
  final = rbindlist(my_data, fill =T)
  My.Kable.All(final)
  
  if (file.exists(paste0(CCPRData, 'ccpr_report_detail_files.rds')))
  {
    data_old = CCPR_READRDS(CCPRData,'ccpr_report_detail_files.rds' , ToRestore = T)
  } else {data_old = data.table()}
  data_final =  unique(rbind(final, data_old, fill =T) [order (-upload)], by = c('file','dataset'))
  CCPR_SAVERDS(data_final, CCPRData, 'ccpr_report_detail_files.rds', ToKable = T, SaveOneDrive = T)
  
  CCPR_FTP_UPLOAD_FOLDER_FILE(File_Folder='R:\\\\CCPR\\\\DATA\\\\', File_Name=tolower('ccpr_report_detail_files.rds'), File_To=tolower('ccpr_report_detail_files.rds'))
  # CREATE_JQGRID_API_URL(pFolder=paste0(CCPRData), FileName = tolower('ccpr_report_detail_files.rds'))
}

# ==================================================================================================
CCPR_INDCHART_FOR_WEB_BYFILE  = function ( pFolder = 'R:/CCPR/DATA/FOR_WEB/', FileLocal = 'ccpr_indvn_history' , FileUpLoad ='for_web_indvn_chart.rds' ) {
  # ------------------------------------------------------------------------------------------------
  # pcode = 'INDSPX'
  try(IFRC.UPLOAD.RDS.2023(l.host = "ccpr.vn", l.tablename= tolower(FileLocal),
                           l.filepath= tolower(paste0('R:/CCPR/DATA/FOR_WEB/',  paste0(FileLocal,'.rds'))),
                           CheckField=T, ToPrint = T, ToForceUpload=T, ToForceStructure=T))
  CCPR_FTP_UPLOAD_FOLDER_FILE(File_Folder='R:\\\\CCPR\\\\DATA\\\\FOR_WEB\\\\', File_Name=tolower(paste0(FileLocal, '.rds')), File_To=tolower(paste0(FileLocal, '.rds')))
  Sys.sleep(2)
  ccpr_indtest_history <- readRDS(paste0(pFolder ,FileLocal, '.rds'))
  # ccpr_indtest_history [code  == 'INDVNINDEX']
  # pcode = 'INDVNINDEX'
  list_ind = unique(ccpr_indtest_history, by = 'code' )
  x = list() 
  for (i in 1: nrow(list_ind))
  {
    # i = 1
    pcode = list_ind[i]$code
    piso2 = ccpr_indtest_history[code == pcode]$iso2 [1]
    pcountry = ccpr_indtest_history[code == pcode]$country [1]
    name = ccpr_indtest_history[code == pcode]$name [1]
    date = max(ccpr_indtest_history[code == pcode]$date)
    start = min(ccpr_indtest_history[code == pcode]$date)
    start_date = year(start)
    end_date = year(date)
    pname = gsub(' ','%20', gsub('&','%26',name))
    subtitle = paste0('LAST%20UPDATE:%20',date)
    
    chart1 = paste0('https://investment.ccpr.vn/api/chart/line_row/demo@ccpr.vn:Demo2023?table=',FileLocal,'&title=',pname
                    ,'%20INDEX&title_size=19&title_color=FAFAFA&sub_title=',subtitle,'&sub_title_size=16&sub_title_color=FAFAFAf&series=',pcode
                    ,'&series_lable=',pname,'&series_color=Blue&start_date=',start_date,'&end_date=',end_date,'&format_date=yyyy-mm')
    if (pcode != 'INDVNINDEX') {
    chart2 = paste0('https://investment.ccpr.vn/api/chart/line_row/demo@ccpr.vn:Demo2023?table=',FileLocal,'&type_lines=start&title=COMPARE%20',pname
                    ,'%20AND%20VNI%20INDEX&title_size=19&title_color=FAFAFA&sub_title=',subtitle,'&sub_title_size=16&sub_title_color=FAFAFAf&series=',pcode,
                    ',INDVNINDEX&series_lable=',pname,',VNI&series_color=Blue,Red&start_date=2008',
                    '&end_date=',end_date,'&format_date=yyyy-mm&yAxis_unit=%&percent=T') } else {chart2 = NA}
    if (pcode != 'INDSPX'  ){
      if (pcode != 'INDVNINDEX') {
        chart3 = paste0( 'https://investment.ccpr.vn/api/chart/line_row/demo@ccpr.vn:Demo2023?table=',FileLocal,'&type_lines=start&title=COMPARE%20',pname
                         ,',%20VNI%20AND%20S%26P500%20INDEXES&title_size=19&title_color=FAFAFA&sub_title=',subtitle,'&sub_title_size=16&sub_title_color=FAFAFAf&series=',pcode
                         ,',INDVNINDEX,INDSPX&series_lable=',pname,',VNI,S%26P%20500&series_color=Blue,Red,Yellow&start_date=2000',
                         '&end_date=',end_date,'&format_date=yyyy-mm&yAxis_unit=%&percent=T')      }  else { 
        chart3 = paste0( 'https://investment.ccpr.vn/api/chart/line_row/demo@ccpr.vn:Demo2023?table=',FileLocal,'&type_lines=start&title=COMPARE%20VNI%20AND%20S%26P500%20INDEXES&title_size=19&title_color=FAFAFA&sub_title=',subtitle,'&sub_title_size=16&sub_title_color=FAFAFAf&series=',pcode
                         ,',INDSPX&series_lable=',pname,',S%26P%20500&series_color=Blue,Red,Yellow&start_date=2000',
                         '&end_date=',end_date,'&format_date=yyyy-mm&yAxis_unit=%&percent=T') }  }   else {chart3 = NA} 
    
    detail = paste0('https://datacenter.ccpr.vn/api/get_file/demo@ccpr.vn:Demo2023?filerds=',paste0(FileLocal, '.rds'),
                    '&fields=iso2,country,continent,type,name,code,date,close,rt,rtm,rty&label=ISO2,COUNTRY,CONTINENT,TYPE,NAME,CODE,DATE,CLOSE,RT,MTD,YTD&width=5,12,10,5,30,20,10,10,5,5,5&format=c,c,c,c,c,c,d,n3,p2,p2,p2&title=',
                    pname, '%20HISTORY&code=',pcode,'&note=YTD%20=%20Year%20To%20Date')
    upload = substr(Sys.time(), 1, 16)
    data_new = data.table(iso2 = piso2, country = pcountry,category = 'IND', code = pcode, name = name,start = start, date = date, detail = detail, chart = chart1, vni = chart2, spx = chart3, upload= upload)
    # sys.sleep (2)
    x [[i]] = data_new
    Sys.sleep (1) 
  }
  # x = indchart ( pcode = 'INDSTOXX50E')
  xy = rbindlist(x, fill =T)
  My.Kable.TB(xy)
  saveRDS(xy, paste0(pFolder, FileUpLoad))
  Sys.sleep (2)
...
# ==================================================================================================
CCPR_FTP_UPLOAD_FOLDER_FILE = function(File_Folder='R:\\\\SECTORS\\\\OVERVIEW_REPORTS\\\\', File_Name='CCPR_WORLD_INDEXES_MONTHLY_REPORT.rds', File_To='ccpr_world_indexes_monthly_report.rds') {
  # ------------------------------------------------------------------------------------------------
  
  File_Raw = paste0('winscp_', gsub(' ','_', gsub(':|-','', substr(Sys.time(),1,19))), '.txt')
  File_Bat = paste0('R:/WINSCP/', File_Raw)
  file.copy(paste0('R:/WINSCP/', 'winscp_model.txt'), File_Bat )
  content = readr::read_file(File_Bat)
  content = gsub('<FILE_FOLDER>', File_Folder, content)
  content = gsub('<FILE_NAME>', File_Name, content)
  content = gsub('<FILE_TO>', File_To, content)
  writeLines(content, File_Bat)
  CommandLine = paste0('start "" "C:\\Program Files (x86)\\WinSCP\\winscp.com" "/script=r:/winscp/', File_Raw, '"')
  CATln_Border(CommandLine)
  shell(CommandLine)
  
}

# ==================================================================================================
IFRC_CCPR_INSTALLED_SOFTWARES = function() {
  # ------------------------------------------------------------------------------------------------
  # x = IFRC_CCPR_INSTALLED_SOFTWARES()
  installed_software <- system("wmic product get name", intern = TRUE)
  installed_software <- installed_software[-1]  # Remove the header
  My_Softwares = as.data.table(installed_software)
  My_Softwares[, installed_software:=trimws(gsub('\n','', installed_software))]
  My_Softwares = My_Softwares[!grepl('Microsoft Visual C|Java',installed_software )][nchar(installed_software)>0]
  cat('\014')
  CATln_Border('INSTALLED SOFTWARES')
  My.Kable.All(My_Softwares)
  return(My_Softwares)
}

# ==================================================================================================
IFRC_CCPR_PC_POWER = function(ToPrint=F) {
  # ------------------------------------------------------------------------------------------------
  # z = IFRC_CCPR_PC_POWER(ToPrint=T)
  MY_PC = toupper(IFRC_CCPR_PC(ToPrint=ToPrint))
  IFRC.CCPR.PC_POWER = 3 # LOW
  if (MY_PC %in% list('IFRC-MASTER', 'IFRC_IREEDS', 'IFRC-3630'))   { IFRC.CCPR.PC_POWER = 1}
  if (MY_PC %in% list('IFRC-MHM', 'BEQ_RD5', 'BEQ-RD4', 'BEQ-RD3')) { IFRC.CCPR.PC_POWER = 2}
  assign('IFRC.CCPR.PC_POWER', as.numeric(IFRC.CCPR.PC_POWER), envir = .GlobalEnv)
  if (ToPrint) { print(IFRC.CCPR.PC_POWER, quote=T) }
  return(IFRC.CCPR.PC_POWER)
}

# ==================================================================================================
IFRC_CCPR_PC = function(ToPrint=F) {
  # ------------------------------------------------------------------------------------------------
  # z = IFRC_CCPR_PC(ToPrint=T)
  if (!exists('IFRC.CCPR.PC'))
  { 
    File_Name = 'c:/r/my_pc.txt'
    if (file.exists(File_Name))
    {
      IFRC.CCPR.PC = toupper(as.character(readLines(File_Name)))
    } else { IFRC.CCPR.PC = 'NONAME'}
    
    assign('IFRC.CCPR.PC', as.character(IFRC.CCPR.PC), envir = .GlobalEnv)
  }
  if (ToPrint) { print(IFRC.CCPR.PC, quote=T) }
  return(IFRC.CCPR.PC)
}

# ==================================================================================================
IFRC_CCPR_CONFIFURATION = function() {
  # ------------------------------------------------------------------------------------------------
  library(memuse)
  # IFRC_CCPR_CONFIFURATION()
  MY_CONFIGURATION = data.table()
  MY_CONFIGURATION = rbind(MY_CONFIGURATION, data.table(name='MY PC',          value=toupper(as.character(IFRC_CCPR_PC(ToPrint=F)))))
  MY_CONFIGURATION = rbind(MY_CONFIGURATION, data.table(name='MY PC POWER',    value=as.character(IFRC_CCPR_PC_POWER(ToPrint=F))))
  MY_CONFIGURATION = rbind(MY_CONFIGURATION, data.table(name='ONEDRIVE DRIVE', value=toupper(as.character(ifelse(exists('ODDrive'), as.character(ODDrive), '-')))))
  MY_CONFIGURATION = rbind(MY_CONFIGURATION, data.table(name='UData',          value=toupper(as.character(ifelse(exists('UData'), as.character(UData), '-')))))
  MY_CONFIGURATION = rbind(MY_CONFIGURATION, data.table(name='CCPRData',       value=toupper(as.character(ifelse(exists('CCPRData'), as.character(CCPRData), '-')))))
  MY_CONFIGURATION = rbind(MY_CONFIGURATION, data.table(name='ODLibrary',      value=toupper(as.character(ifelse(exists('ODLibrary'), as.character(ODLibrary), '-')))))
  x = R.Version()
  MY_CONFIGURATION = rbind(MY_CONFIGURATION, data.table(name='R Version',      value=trimws(word(x$version.string,1,1,'[(]'))))
  x = memuse::Sys.meminfo()
  MY_CONFIGURATION = rbind(MY_CONFIGURATION, data.table(name='Total RAM',      value=trimws(x[[1]])))
  MY_CONFIGURATION = rbind(MY_CONFIGURATION, data.table(name='Free RAM',       value=trimws(x[[2]])))
  ...
  disks <- disks[-1,]
  My_DISKS = as.data.table(disks)
  My_DISKS = CLEAN_COLNAMES(My_DISKS)
  # My_DISKS = My_DISKS[!grepl('U|T', caption)]
  My_DISKS[, size:=as.numeric(size)]
  My_DISKS[, freespace:=as.numeric(freespace)]
  My_DISKS = My_DISKS[!is.na(size)]
  My_DISKS_TO_ADD = My_DISKS[, .(name = paste('DRIVE', caption), value=paste(Format.Number(round(freespace/1000000000),0), 'Gb', 'of',
                                                                             Format.Number(round(size/1000000000),0), 'Gb'))]
  MY_CONFIGURATION = rbind(MY_CONFIGURATION, My_DISKS_TO_ADD)
  My.Kable.All(MY_CONFIGURATION)
}

# ==================================================================================================
MERGE_XLIST_STD = function(xList, ToKable=T, GroupBy ='code x date', Clean_Data_Rt=F) {
  # ----------------------------------------------------------------------------
  xALL = rbindlist(xList, fill=T)
  if (nrow(xALL)>0)
  {
    xALL = MERGE_DATASTD_BYCODE(xALL)
    xALL$capiusd = NULL
    switch(GroupBy,
           'code x date'                   = { xALL = unique(xALL, by=c('code', 'date'))},
           'codesource x date'             = { xALL = unique(xALL, by=c('codesource', 'date'))},
           'codesource x date x timestamp' = { xALL = unique(xALL, by=c('codesource', 'date', 'timestamp'))},
           'code x date x timestamp'       = { xALL = unique(xALL, by=c('code', 'date', 'timestamp'))}
    )
    if (Clean_Data_Rt) { xALL = CLEAN_DATA_RT(xALL) }
    if (ToKable) { My.Kable.TB(xALL) }
  }
  return(xALL)
}

# ==================================================================================================
CCPR.UPLOAD.RDS.2023 = function(l.host = "ifrc.vn", l.tablename, l.filepath, CheckField=T, ToPrint = F,
...
# ==================================================================================================
CCPR_DOWNLOAD_VST_STKVN_SHARES_BY_CODE = function(pCode = 'VND') {
  # ------------------------------------------------------------------------------------------------
  # pCode = 'YRC'
...
# ==================================================================================================
CCPR_DOWNLOAD_C68_STKVN_SHARES_BY_CODE = function(pCode = 'VND') {
  # ------------------------------------------------------------------------------------------------
  # pCode = 'YRC'
  Final_Data = data.table()
...
# ==================================================================================================
CCPR_DOWNLOAD_STB_STKVN_SHARES_BY_CODE = function(pCode = 'VND') {
  # ------------------------------------------------------------------------------------------------
  # pCode = 'VND'
  Final_Data = data.table()
  ...
  writeLines(content, 'c:/temp/content.txt')
  x.SHARESLIS = trimws(gsub('\r\n|</td>', '', str.extract(content, 'KL đang niêm yết', '</tr') ) )
  ...
  Final_Data = data.table(source='STB', ticker=pCode, codesource=pCode, code=paste0('STKVN', pCode), date=SYSDATETIME(10), 
                          shareslis=v.SHARESLIS, updated=substr(as.character(Sys.time()),1,19))
  My.Kable.All(Final_Data)
  return(Final_Data)
}

# ==================================================================================================
CCPR_DOWNLOAD_CAF_STKVN_SHARES_BY_CODE = function(pCode  = 'VNM') {
  # ------------------------------------------------------------------------------------------------
  Final_Data = data.table()
  # RELOAD_INSREF()
  ...
  if (nrow(CAF_ONE)==1)
  {
    ...
    SharesLis = as.numeric(gsub(',','', trimws(word(x.SHARESLIS,2,2,">")))); CATln(x.SHARESLIS)
    x.SHARESOUT = trimws(gsub('\r\n', '', str.extract(content, '<div class="l"><b>KLCP đang lưu hành:</b></div>', '</div')))
    SharesOut = as.numeric(gsub(',','', trimws(word(x.SHARESOUT,2,2,">")))); CATln(x.SHARESOUT)
    LastTrading = CCPR_LAST_TRADING_DAY_VN(max(9, hour(Sys.time())), ToPrompt = T)
...
# ==================================================================================================
UPDATE_FILES_1M_1Y_FROM_HISTORY = function(File_BaseName = 'EFRC_ECO_HISTORY', MinPercent=1) {
  # ------------------------------------------------------------------------------------------------
  try(REPAIR_SUMMARY_RDS(File_Folder=UData, File_RDS = paste0(File_BaseName, '.rds')))
  
  try(REPAIR_SUMMARY_RDS(File_Folder=UData, File_RDS = paste0(File_BaseName, '_1M.rds')))
  try(SMART_QUICK_INTEGRATION_FILES(File_Fr=paste0(File_BaseName, '.rds'), 
                                    File_To=paste0(File_BaseName, '_1M.rds'), 
                                    Field_STD='', IncludedOnly=F, MinPercent=MinPercent, NbDays=31, pCondition=''))
  
  try(REPAIR_SUMMARY_RDS(File_Folder=UData, File_RDS = paste0(File_BaseName, '_1Y.rds')))
  try(SMART_QUICK_INTEGRATION_FILES(File_Fr=paste0(File_BaseName, '.rds'), 
                                    File_To=paste0(File_BaseName, '_1Y.rds'), 
                                    Field_STD='', IncludedOnly=F, MinPercent=MinPercent, NbDays=251, pCondition=''))
}

# ==================================================================================================
UPDATE_FILES_HISTORY_FROM_1M_1Y = function(File_BaseName = 'EFRC_ECO_HISTORY', MinPercent=1) {
  # ------------------------------------------------------------------------------------------------
  try(REPAIR_SUMMARY_RDS(File_Folder=UData, File_RDS = paste0(File_BaseName, '.rds')))
  ...
  try(SMART_QUICK_INTEGRATION_FILES(File_Fr=paste0(File_BaseName, '_1M.rds'), 
                                    File_To=paste0(File_BaseName, '_1Y.rds'), 
                                    Field_STD='', IncludedOnly=F, MinPercent=MinPercent, NbDays=251, pCondition=''))
  
  try(SMART_QUICK_INTEGRATION_FILES(File_Fr=paste0(File_BaseName, '_1Y.rds'), 
                                    File_To=paste0(File_BaseName, '.rds'), 
                                    Field_STD='', IncludedOnly=F, MinPercent=MinPercent, NbDays=0, pCondition=''))
}

# ==================================================================================================
UPDATE_FILES_SYNCHRONISE_1M_1Y = function(File_BaseName = 'EFRC_ECO_HISTORY', MinPercent=1) {
  # ------------------------------------------------------------------------------------------------
  try(REPAIR_SUMMARY_RDS(File_Folder=UData, File_RDS = paste0(File_BaseName, '.rds')))
...
# ==================================================================================================
REPAIR_SUMMARY_RDS = function(File_Folder=UData, File_RDS = 'EFRC_COV_HISTORY_1M.rds') {
  # ------------------------------------------------------------------------------------------------
  if (file.exists(paste0(File_Folder, File_RDS)))
  {
    File_Summary = CHECK_CLASS(try(CCPR_LOADRDS_SUMMARY(File_Folder, File_RDS)))
...
# ==================================================================================================
CCPR_LOADRDS_SUMMARY = function(pFolder=UData, pFileRDS='IFRC_CUR_4INDEX.rds') {
  # ------------------------------------------------------------------------------------------------
...
# ==================================================================================================
VERY_QUICK_DATA_INTEGRATION_CODESOURCExDATE = function(pData=Final_Data, Folder_To=UData, File_To='IFRC_SCREENER_STKTH_LISTRAW_HISTORY.rds') {
  # ------------------------------------------------------------------------------------------------
  File_Name = File_To
  Data_Hst  = CCPR_READRDS(Folder_To, File_Name, ToKable = T)
  # pData = Final_Data
  # if (nrow(pData)>0)
  # {
  #   if (!'timestamp' %in% names(pData) & 'datetime' %in% names(pData)) { pData[, timestamp:=substr(datetime,1,19)] }
  #   if (!'timestamp' %in% names(pData)) { pData[, timestamp:=as.character(timestamp)] }
  #   if ('datetime' %in% names(pData)) { pData[, datetime:=as.character(datetime)] }
  # }
  # 
  # if (nrow(Data_Hst)>0)
  # {
  #   if (!'timestamp' %in% names(Data_Hst) & 'datetime' %in% names(Data_Hst)) { Data_Hst[, timestamp:=substr(datetime,1,19)] }
  #   if (!'timestamp' %in% names(Data_Hst)) { Data_Hst[, timestamp:=as.character(timestamp)] }
  #   if ('datetime' %in% names(Data_Hst)) { Data_Hst[, datetime:=as.character(datetime)] }
  # }
  # str(Data_Old)
  # str(xALL)
  N0 = nrow(Data_Hst)
  Data_Hst  = unique(rbind(Data_Hst, pData, fill=T), FromLast=T, by=c('codesource', 'date'))
  N1 = nrow(Data_Hst)
  CATln_Border(paste('NEW RECORDS = ', Format.Number(N1-N0,0)))
  if (nrow(Data_Hst)>0)
  {
    My.Kable.Min(Data_Hst)
    try(CCPR_SAVERDS(Data_Hst, UData, File_Name, SaveOneDrive = T))
  }
}

# ==================================================================================================
VERY_QUICK_DATA_INTEGRATION_INTRADAY = function(pData=xALL, Folder_To=UData, File_To='DOWNLOAD_YAH_CUR_INTRADAY_HISTORY.rds') {
  # ------------------------------------------------------------------------------------------------
  File_Name = File_To
  Data_Hst  = CCPR_READRDS(Folder_To, File_Name, ToKable = T)
  ...
  # str(xALL)
  N0 = nrow(Data_Hst)
  Data_Hst  = unique(rbind(Data_Hst, pData, fill=T), FromLast=T, by=c('code', 'date', 'timestamp'))
  N1 = nrow(Data_Hst)
  CATln_Border(paste('NEW RECORDS = ', Format.Number(N1-N0,0)))
  My.Kable.Min(Data_Hst)
  CCPR_SAVERDS(Data_Hst, UData, File_Name, ToSummary = T, SaveOneDrive = T)
}
...
My.Kable.Min = function(pData, MinFields = c('provider', 'type', 'fcat', 'scat', 'country', 'iso2', 'source', 'codesource', 
                                             ...
                                             'reference', 'dataset', 'datatype', 'close', 'close_adj', 'volume', 'rt')) {
  # ------------------------------------------------------------------------------------------------
  ...
  My.Kable.TB(pData[, ..CommonFields])
}

# ==================================================================================================
IFRC_DOWNLOAD_C68_STKVN_PRICES_BY_CODE = function(pCode = 'VIC') {
  # ------------------------------------------------------------------------------------------------
  DATACENTER_LINE_BORDER(paste('IFRC_DOWNLOAD_C68_STKVN_PRICES_BY_CODE :',pCode))
  Final_Data = data.table()
  pURL  = paste0('https://www.cophieu68.vn/quote/history.php?id=', pCode)
  # pURL  = paste0('https://www.casablanca-bourse.com/en/live-market/instruments/', pCode, '?scrollTo=instrument-histo-var')
  CATln_Border(pURL)
  content    = try(rvest::read_html(pURL))
  if (all(class(content)!='try-error'))
  {
    ...
    xX3 = gsub('\t','', xData[2]$x3)
    xClose  = word(xX3,1,1, '\r\n')
    xChange = word(xX3,2,2, '\r\n')
    Final_Data = xData[, .(codesource=pCode, code=paste0('STKVN', pCode), source='C68', 
                           date=as.Date(x2, '%d/%m/%Y'), 
                           close     = 1000*as.numeric(gsub(',','', word(gsub('\t','', x3),1,1, '\r\n'))), 
                           open      = 1000*as.numeric(gsub(',','', x5)), 
                           high      = 1000*as.numeric(gsub(',','', x6)), 
                           low       = 1000*as.numeric(gsub(',','', x7)), 
                           volume    = as.numeric(gsub(',','', x4)),
                           for_buy   = as.numeric(gsub(',','', x8)), 
                           for_sell  = as.numeric(gsub(',','', x9)),
                           change    = 1000*as.numeric(word(x3,2,2, '\r\n'))
                           # varpc     = as.numeric(gsub(',','', gsub('\\(|)|%','', x5))) 
    )
    ]
    
    Final_Data = Final_Data[!is.na(close)]
    Final_Data[, reference:=(close-change)]
    Final_Data[, rt:=(change)/reference]
    # My.Kable(Final_Data)
    
    # Final_Data[close_adj==0, close_adj:=as.numeric(NA)]
    # Final_Data[, ref_calc:=close-change]
    # if ('reference' %in% names(Final_Data)) { Final_Data[, rt:=(close/reference)-1] }
    # Final_Data = Final_Data[order(date)]
    Final_Data[, rt:=(close/reference)-1]
    Final_Data = MERGE_DATASTD_BYCODE(Final_Data, pOption='FINAL')
    My.Kable.Min(Final_Data)
  }
  DATACENTER_LINE_BORDER_BEGINEND(paste('IFRC_DOWNLOAD_C68_STKVN_PRICES_BY_CODE :',pCode), 'END')
  return(Final_Data) 
}

# ==================================================================================================
CAFEF_EXTRACT_INFO = function(pWeb, SearchStr, EndStr, LastSep=' ', IsNumeric=T, ToPrint=F) {
  # ------------------------------------------------------------------------------------------------
  # pWeb = MyWeb
  # SearchStr = '<div class="l"><b>KLCP dang niem yet:</b></div>'
  # EndStr    = '</div>'
  My_Result  = ''
  My_Extract = REPLACE_LOOP_ALL(trimws(gsub('  ', ' ', gsub('\n',' ', str.extract(pWeb, SearchStr, EndStr)))), '  ', ' ')
  My_Extract
  
  nBlank = lengths(regmatches(My_Extract, gregexpr(LastSep, My_Extract)))
  
  My_Result = word(My_Extract,nBlank+1,nBlank+1, sep = LastSep)
  if (IsNumeric)
  {
    My_Result = as.numeric(gsub(',','',My_Result))
  }
  if (ToPrint) { print(My_Result, quote=F)}
  return(My_Result)
}

# ==================================================================================================
DOWNLOAD_CAF_STKVN_OVERVIEW = function(pCode='VIC', pURL     = 'https://s.cafef.vn/hose/vic-tap-doan-vingroup-cong-ty-co-phan.chn', ToBrowse=F) {
  # ------------------------------------------------------------------------------------------------
  
  # content  = try(rvest::read_html(pURL))
  # tables   = content %>% html_table(fill = TRUE)
  # xTable   = SELECT_TABLE_WITH_FIELD(tables=tables, pField='x1', pContent="Chairman", like=T)
  Final_Data = data.table()
  MyExecution = gsub('<CODE>', pCode, 'python r:/python/savehtml_edit.py "<URL>" "r:/python/cafef_shares.html"')
  MyExecution = gsub('<URL>',  pURL, 'python r:/python/savehtml_edit.py "<URL>" "r:/python/cafef_shares.html"')
  CATln_Border(MyExecution)
  shell(MyExecution)
  
  file.exists("r:/python/cafef_shares.html")
  content  = try(rvest::read_html("r:/python/cafef_shares.html"))
  if (all(class(content)!='try-error'))
  {
    MyWeb = toString(content)
    MyWeb = decodeVN(MyWeb, from='Unicode', to='Unicode', diacritics=F)
    # writeLines(MyWeb, 'r:/python/cafef_web.html')
    
    SharesOut = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='<div class="l"><b>KLCP dang luu hanh:</b></div>', EndStr='</div>', LastSep='>', IsNumeric=T, ToPrint=ToBrowse)
    SharesLis = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='<div class="l"><b>KLCP dang niem yet:</b></div>', EndStr='</div>', LastSep='>', IsNumeric=T, ToPrint=ToBrowse)
    First_TradingDate = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='Ngay giao dich dau tien:', EndStr='</b>', LastSep='>', IsNumeric=F, ToPrint=ToBrowse)
    First_TradingClose = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='Gia dong cua phien GD dau tien(nghin dong):', EndStr='</b>', LastSep='>', IsNumeric=T, ToPrint=ToBrowse)
    First_TradingSharesLis = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='Khoi luong co phieu niem yet lan dau:', EndStr='</b>', LastSep='>', IsNumeric=T, ToPrint=ToBrowse)
    Price_High     = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='<div class="l">Gia cao nhat</div>', EndStr='</div>', LastSep='>', IsNumeric=T, ToPrint=ToBrowse)
    Price_Low      = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='<div class="l">Gia thap nhat</div>', EndStr='</div>', LastSep='>', IsNumeric=T, ToPrint=ToBrowse)
    Price_Ref      = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='<div class="l">Gia tham chieu</div>', EndStr='</div>', LastSep='>', IsNumeric=T, ToPrint=ToBrowse)
    Price_Open     = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='<div class="l">Gia mo cua</div>', EndStr='</div>', LastSep='>', IsNumeric=T, ToPrint=ToBrowse)
    Price_Last     = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='<span id="ltrCurentPrice">', EndStr='</div>', LastSep='>', IsNumeric=T, ToPrint=ToBrowse)
    Volume         = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='<div class="v1">Khoi luong</div>', EndStr='</div>', LastSep='>', IsNumeric=T, ToPrint=ToBrowse)
    
    Price_Updated  = CAFEF_EXTRACT_INFO(pWeb=MyWeb, SearchStr='<div>Cap nhat:</div>', EndStr='</div>', LastSep='>', IsNumeric=F, ToPrint=ToBrowse)
    Price_Date     = as.Date(word(Price_Updated,2,2,','), '%d/%m/%Y')
    Price_Time     = as.character(word(Price_Updated,1,1,' '))
    Price_DateTime = paste(Price_Date, paste0(Price_Time, ':00'))
    
    Final_Data = data.table(codesource= pCode, 
                            code      = paste0('STKVN', pCode), 
                            source    = 'CAF', 
                            date      = Price_Date, 
                            time      = Price_Time,
                            datetime  = Price_DateTime,
                            close     = 1000*as.numeric(gsub(',','', Price_Last)), 
                            open      = 1000*as.numeric(gsub(',','', Price_Open)), 
                            high      = 1000*as.numeric(gsub(',','', Price_High)), 
                            low       = 1000*as.numeric(gsub(',','', Price_Low)), 
                            reference = 1000*as.numeric(gsub(',','', Price_Ref)), 
                            volume    = as.numeric(gsub(',','', Volume)), 
                            first_date    = as.Date(First_TradingDate, '%d/%m/%Y'), 
                            first_shares  = as.numeric(gsub(',','', First_TradingSharesLis)), 
                            sharesout = as.numeric(gsub(',','', SharesOut)), 
                            shareslis = as.numeric(gsub(',','', SharesLis)))
    
    Final_Data[, rt:=(close/reference)-1]
    My.Kable.All(Final_Data[, -c('datetime')])
  }
  return(Final_Data)
}

# ==================================================================================================
IFRC_DOWNLOAD_HSX_STKVN_REF = function(pFolder=CCPRData, ToIntegrateHistory=T) {
  # ------------------------------------------------------------------------------------------------
  # x = IFRC_DOWNLOAD_HSX_STKVN_REF(pFolder=UData, ToIntegrateHistory=T)
  ...
  if (nrow(data_group)>0) 
  {
    ...
    CCPR_SAVERDS(data_group, pFolder, "download_hsx_stkvn_ref.rds", ToSummary=T, SaveOneDrive = T)
    
    if (ToIntegrateHistory) 
    {
      Data_Old = CCPR_READRDS(pFolder, 'download_hsx_stkvn_ref_history.rds', ToKable = T, ToRestore = T)
      ...
      CCPR_SAVERDS(Data_Old, pFolder, 'download_hsx_stkvn_ref_history.rds', ToSummary = T, SaveOneDrive = T)
    }
  }
  # ............................................................................
  return(data_group)
...
# ==================================================================================================
IFRC_DOWNLOAD_C68_STKVN_REF_SHARES_DAY = function(pFolder=UData, ToIntegrateHistory=F) {
  # ------------------------------------------------------------------------------------------------
  # IFRC_DOWNLOAD_C68_STKVN_REF_SHARES_DAY(pFolder=UData, ToIntegrateHistory=T)
  All_Data    = data.table()
  
  pMarket     = 'UPC'
  LastTrading = CCPR_LAST_TRADING_DAY_VN(substr(Sys.time(),12,13), ToPrompt = T)
  pURL  = 'https://www.cophieu68.vn/market/markets.php?id=^upcom'
  ...
  if (all(class(content)!='try-error'))
  {
    ...
    xData      = as.data.table(tables[[2]])
    xData      = CLEAN_COLNAMES(xData)
    xData_Header = xData[1:1]
    xData = xData[2:nrow(xData)]
    str(xData)
    Final_Data = xData[, .(
      source = 'C68', date=LastTrading, market = pMarket,
      close  = as.numeric(gsub(',','', as.character(x2)))*1000,
      change = as.numeric(gsub(',','', as.character(x3)))*1000,
      volume = as.numeric(gsub(',','', as.character(x4))),
      shareslis = as.numeric(gsub(',','', as.character(x6))),
      codesource=toupper(word(gsub('\t','', x1),3,3, '\n'))
    )]
    Final_Data[, reference:=close-change]
    Final_Data[, rt:=change/reference]
    ...
    Final_Data = MERGE_DATASTD_BYCODE(Final_Data, pOption='FINAL')
    My.Kable(Final_Data)
    CCPR_SAVERDS(Final_Data, UData, 'download_c68_upc_stkvn_ref.rds', ToSummary = T, SaveOneDrive = T)
  }
  All_Data = rbind(All_Data, Final_Data, fill=T)
  
  pMarket     = 'HNX'
  LastTrading = CCPR_LAST_TRADING_DAY_VN(substr(Sys.time(),12,13), ToPrompt = T)
  pURL  = 'https://www.cophieu68.vn/market/markets.php?id=^hastc'
  x     = WEB_SAVE_AS(pURL = pURL, SaveFolder = 'c:/python/downloads/')
  content    = try(rvest::read_html(x))
  if (all(class(content)!='try-error'))
  {
    ...
    Final_Data = MERGE_DATASTD_BYCODE(Final_Data, pOption='FINAL')
    My.Kable(Final_Data)
    CCPR_SAVERDS(Final_Data, UData, 'download_c68_hnx_stkvn_ref.rds', ToSummary = T, SaveOneDrive = T)
  }
  All_Data = rbind(All_Data, Final_Data, fill=T)
  
  pMarket     = 'HSX'
  LastTrading = CCPR_LAST_TRADING_DAY_VN(substr(Sys.time(),12,13), ToPrompt = T)
  pURL  = 'https://www.cophieu68.vn/market/markets.php?id=^vnindex'
  x     = WEB_SAVE_AS(pURL = pURL, SaveFolder = 'c:/python/downloads/')
  content    = try(rvest::read_html(x))
  if (all(class(content)!='try-error'))
  {
    ...
    Final_Data = MERGE_DATASTD_BYCODE(Final_Data, pOption='FINAL')
    My.Kable(Final_Data)
    CCPR_SAVERDS(Final_Data, UData, 'download_c68_hsx_stkvn_ref.rds', ToSummary = T, SaveOneDrive = T)
  }
  All_Data = rbind(All_Data, Final_Data, fill=T)
  # pFolder = UData
  CCPR_SAVERDS(All_Data, pFolder, 'download_c68_stkvn_ref_day.rds', ToSummary = T, SaveOneDrive = T)
  CCPR_SAVERDS(All_Data, pFolder, 'download_c68_stkvn_ref.rds', ToSummary = T, SaveOneDrive = T)
  CCPR_SAVERDS(All_Data, pFolder, 'download_c68_stkvn_shares_day.rds', ToSummary = T, SaveOneDrive = T)
  # ...
  CCPR_SAVERDS(All_Data, pFolder, 'IFRC_DOWNLOAD_C68_STKVN_REF_DAY.rds', ToSummary = T, SaveOneDrive = T)
  CCPR_SAVERDS(All_Data, pFolder, 'IFRC_DOWNLOAD_C68_STKVN_SHARES_DAY.rds', ToSummary = T, SaveOneDrive = T)
  if (ToIntegrateHistory) 
  {
    QUICK_INTEGRATION(File_Fr = 'IFRC_DOWNLOAD_C68_STKVN_SHARES_DAY.rds', File_To = 'IFRC_DOWNLOAD_C68_STKVN_SHARES_HISTORY.rds')
    QUICK_INTEGRATION(File_Fr = 'IFRC_DOWNLOAD_C68_STKVN_REF_DAY.rds', File_To = 'IFRC_DOWNLOAD_C68_STKVN_REF_HISTORY.rds')
  }
}

# ==================================================================================================
IFRC_DATA_QUICK_REPORT = function(pData, list_all=F) {
  # ------------------------------------------------------------------------------------------------
  RELOAD_INSREF()
  pData = pData[order(code, date)]
  pData[,nbdays:=as.numeric(date-shift(date)), by='code']
  Data_Report = pData[, .(name=name[1],start=date[1], end=date[.N], last=close[.N], nbdays=nbdays[.N], records=.N), by='code'][order(end)]
  Data_Report = merge(Data_Report, ins_ref[, .(code, iso2, country, continent, fcat, scat, home, sample)], all.x=T, by='code')
  
  if (list_all) {  My.Kable.All(Data_Report) } else {  My.Kable.TB(Data_Report) }
  
  Data_Summary =  Data_Report[, .(nb_codes=.N, nb_records=sum(records, na.rm=T), 
                                  start=min(start, na.rm = T), end=max(end, na.rm=T), 
                                  min_days=min(nbdays, na.rm = T), max_days=max(nbdays, na.rm = T))]

   My.Kable.All(Data_Summary)
   if ('iso2' %in% names(pData)) { print(unique(pData$iso2)) }
   if ('type' %in% names(pData)) { print(unique(pData$type)) }
   if ('source' %in% names(pData)) { print(unique(pData$source)) }
}

# ==================================================================================================
IFRC_DATA_QUICK_REPORT_CODESOURCE = function(pData, list_all=F) {
  # ------------------------------------------------------------------------------------------------
  RELOAD_INSREF()
  pData = pData[order(code, date)]
  pData[,nbdays:=as.numeric(date-shift(date)), by='codesource']
  Data_Report = pData[, .(name=name[1],start=date[1], end=date[.N], last=close[.N], nbdays=nbdays[.N], records=.N), by='codesource'][order(end)]
  Data_Report = merge(Data_Report, ins_ref[, .(codesource=yah, iso2, country, continent, fcat, scat, home, sample)], all.x=T, by='codesource')
...
# ==================================================================================================
CLEAN_DATA_RT = function(pData, ToKable=F) {
  # ------------------------------------------------------------------------------------------------
  # pData = Data_Source
  XXX_HISTORY = copy(pData)
  XXX_HISTORY[type!='STK', close_adj:=close]
  XXX_HISTORY[type!='STK', close_unadj:=close]
  XXX_HISTORY = XXX_HISTORY[order(code, date)]
  XXX_HISTORY[, ':='(nbdays = as.numeric(date-shift(date)))]
  if ('rt' %in% names(XXX_HISTORY)) {  XXX_HISTORY[!is.na(rt), ':='(var=rt, varpc=100*rt)] }
  XXX_HISTORY[, ':='(updated=substr(Sys.time(),1,19))]
  
  if ('type' %in% names(XXX_HISTORY))
  {
    XXX_HISTORY[is.na(rt), ':='(
                change = (close_adj-shift(close_adj)),
                rt     = (close_adj/shift(close_adj))-1, 
                lnrt   = log((close_adj/shift(close_adj)))
                             ) , by='code']
    XXX_HISTORY[, ':='(var=rt, varpc=100*rt)]
  }

  # ToKable = T
  if (ToKable)
  {
    My.Kable.MaxCols(XXX_HISTORY[, -c('capiusd', 'country', 'continent')])
  }
  return(XXX_HISTORY)
}

# ==================================================================================================
CCPR_SAVERDS = function(pData=My_Data, FileFolder=CCPRData, FileName='download_ssi_stkvn_ref_history.rds' , 
...
# ==================================================================================================
CHECK_FILE_QUALITY_IFRC = function(ToReset=F) {
  # ------------------------------------------------------------------------------------------------
  # try(CHECK_FILE_QUALITY_IFRC())
  FileFolder = 'R:/CCPR/MONITOR/'
  FileName   = 'CHECK_FILE_QUALITY_IFRC.rds'
  ToReset    = T
  List_Files = list('EFRC_IND_HISTORY.rds', 'EFRC_INDHOME_HISTORY.rds', 'EFRC_INDSAMPLE_HISTORY.rds',
                    'EFRC_STK_HISTORY.rds', 'EFRC_STKHOME_HISTORY.rds', 'EFRC_STKSAMPLE_HISTORY.rds',
                    'EFRC_BAS_HISTORY.rds', 'EFRC_BASHOME_HISTORY.rds', 'EFRC_BASSAMPLE_HISTORY.rds',
                    'EFRC_BND_HISTORY.rds', 'EFRC_CUR_HISTORY.rds', 'EFRC_CURCRY_HISTORY.rds',
                    'EFRC_FND_HISTORY.rds', 'EFRC_CMD_HISTORY.rds', 'EFRC_FUT_HISTORY.rds', 
                    'EFRC_COV_HISTORY.rds', 'EFRC_CLI_HISTORY.rds', 'EFRC_ECO_HISTORY.rds', 'EFRC_MIR_HISTORY.rds', 'EFRC_STA_HISTORY.rds',
                    'DOWNLOAD_YAH_STK_HISTORY.rds', 'DOWNLOAD_RTS_STK_HISTORY.rds', 
                    'DOWNLOAD_EIK_STK_HISTORY.rds', 'DOWNLOAD_BLG_STK_HISTORY.rds', 
                    'DOWNLOAD_INV_STK_HISTORY.rds', 'DOWNLOAD_GOG_STK_HISTORY.rds',
                    'DOWNLOAD_YAH_IND_HISTORY.rds', 'DOWNLOAD_RTS_IND_HISTORY.rds', 
                    'DOWNLOAD_EIK_IND_HISTORY.rds', 'DOWNLOAD_BLG_IND_HISTORY.rds', 
                    'DOWNLOAD_INV_IND_HISTORY.rds', 'DOWNLOAD_GOO_IND_HISTORY.rds',
                    'DOWNLOAD_YAH_BAS_HISTORY.rds', 'DOWNLOAD_RTS_BAS_HISTORY.rds', 
                    'DOWNLOAD_EIK_BAS_HISTORY.rds', 'DOWNLOAD_BLG_BAS_HISTORY.rds', 
                    'DOWNLOAD_INV_BAS_HISTORY.rds', 'DOWNLOAD_GOG_BAS_HISTORY.rds',
                    'DOWNLOAD_IFRC_IND_HISTORY.rds', 'DOWNLOAD_IFRC_IND_HISTORY_1M.rds', 'DOWNLOAD_IFRC_IND_HISTORY_1M.rds',  
                    'DOWNLOAD_IFRC_INDVN_HISTORY.rds', 'DOWNLOAD_IFRC_INDIN_HISTORY.rds', 
                    'DOWNLOAD_IFRC_INDALS_HISTORY.rds', 'DOWNLOAD_IFRC_INDVNX_HISTORY.rds', 'DOWNLOAD_IFRC_INDPRV_HISTORY.rds', 'DOWNLOAD_IFRC_INDPOR_HISTORY.rds',
                    'DOWNLOAD_IFRC_INDVLI_HISTORY.rds', 'DOWNLOAD_IFRC_INDFGD_HISTORY.rds',
                    'IFRC_INS4INDEX_HISTORY.rds', 'IFRC_STKVN_4INDEX.rds', 'IFRC_CUR_4INDEX.rds')
  ...
  {
    # k  = 1
    qf  = CPPR_FILE_QUALITY_FROM_SUMMARY(pFolder=UData, pFileRDS=List_Files[[k]], ToBrowse=F)
    # df[, pend:=as.Date(pend)]
    # df[, start:=as.Date(start)]
    # df[, end:=as.Date(end)]
    # str(qf)
    Files_All[[k]] = qf
  }
  ...
  setnames(All_Files, c('folder', 'file'), c('location', 'files'), skip_absent = T)
  setnames(All_Files, c('nb_codes', 'nb_end', 'percent_end'), c('codes', 'codes_updated', 'updated_pct'),  skip_absent = T)
  ...
  if (!ToReset)
  {
    Old_Files = CCPR_READRDS(FileFolder,FileName, ToKable = T, ToRestore = T)
  ...
  Final_Files = Final_Files[grepl('^U', location)]
  My.Kable.All(Final_Files)
  SAVERDS_WITH_ONEDRIVE(Final_Files, FileFolder,FileName, SaveOneDrive = T)
}

# ==================================================================================================
CCPR_MONITOR_JOB = function(JOB_ID='GOOD MORNING', pStatus='BEGIN') {
  # ------------------------------------------------------------------------------------------------
  # pStatus='END'
  File_Name = 'CPPR_MONITOR_JOB.rds'
  File_Full = paste0(CCPRDrive, 'CCPR/MONITOR/', File_Name)
  CATln_Border(File_Full)
  if (file.exists(File_Full))
  {
    Monitor_Data = CCPR_READRDS(paste0(CCPRDrive, 'CCPR/MONITOR/'), File_Name, ToRestore = T)
  } else {
    Monitor_Data = data.table()
  }
  
  switch(pStatus,
         'BEGIN' = { Monitor_One = data.table(pc=pMyPC, code=JOB_ID, date=Sys.Date(), status='BEGIN', start=substr(Sys.time(),1,19), updated=substr(Sys.time(),1,19)) },
         'END' =   { 
           if (nrow(Monitor_Data)>0)
           {
             Monitor_One = Monitor_Data[pc==pMyPC & code==JOB_ID]
             if (nrow(Monitor_One)>0)
             {
               Monitor_One[, ':='(end=substr(Sys.time(),1,19), updated=substr(Sys.time(),1,19))]
             } else {
               Monitor_One = data.table(pc=my.pc, code=JOB_ID, date=Sys.Date(), status='BEGIN', start=substr(Sys.time(),1,19), 
                                        end=substr(Sys.time(),1,19), updated=substr(Sys.time(),1,19))
             }
           }
         }
  )
  Monitor_Data = unique(rbind(Monitor_Data, Monitor_One, fill=T)[order(code, -updated)], by=c('code', 'pc'))
  My.Kable.TB(Monitor_Data)
  ToSave = (runif(1,1,100)>80)
  SAVERDS_WITH_ONEDRIVE(Monitor_Data, paste0(CCPRDrive, 'CCPR/MONITOR/'), File_Name, ToSummary=ToSave, SaveOneDrive = ToSave)
  sort(unique(ins_ref$type))
}
...
CPPR_FILE_QUALITY_FROM_SUMMARY = function(pFolder=CCPRData, pFileRDS='DOWNLOAD_HNX_STKVN_SHARES_DAY.RDS', ToBrowse=F) {
  ...
  if (file.exists(paste0(pFolder, pFileSUM)))
  {
    ...
    My_Summary_Stats = My_Summary[, .(folder=pFolder, file=pFileRDS, nb_codes=as.numeric(.N), records=as.numeric(sum(nb, na.rm=T)), 
                                      start=as.Date(min(start, na.rm=T)), end=as.Date(max(datelast, na.rm = T)), 
                                      nb_end=as.numeric(sum(ifelse(date==MaxDate,1,0))), pend=as.Date(MaxpDate), 
                                      nb_pend=as.numeric(sum(ifelse(date==MaxpDate,1,0))))]
...
XLS_UPDATE_BATCH_FILES_MONITOR_CCPR_RDS = function(ToReset=F) {
  ...
  # x1 = try(tcltk2::tk2dde.request("Excel", 'MONITOR_FILES', "R1C1"), silent = T)
         ...
         FileName = 'CHECK_FILE_QUALITY_CCPR.rds' ; FileFolder = paste0(CCPRDrive, 'CCPR/MONITOR/')
         Offset_Row = 3
         Offset_Col = 0

  FullPath_Local = paste0(FileFolder, FileName)
  FullPath_ODDrive = paste0(ODDrive, gsub(':','',FileFolder),FileName)
  # CCPR_READRDS(FileFolder,FileName, ToKable = T, ToRestore = T)
  
  # NEW ........................................................................
  My_EXCEL = openxlsx::read.xlsx(paste0(CCPRDrive, 'CCPR/MONITOR/','MONITOR_CCPR.xlsx'), sheet='FILE_QUALITY', startRow=3)
  My_EXCEL = setDT(My_EXCEL)
  My_EXCEL = My_EXCEL[,1:2]
  # switch(pMyPC,
  #        'BeQ-RD3' = {My_EXCEL = My_EXCEL[,1:2]     }
  #        ,
  #        'BeQ-RD5' = {My_EXCEL = My_EXCEL[,9:10]     }
  # )
  ...
  for (k in 1:nrow(My_EXCEL))
  {
    ...
    qf  = CPPR_FILE_QUALITY_FROM_SUMMARY(pFolder=My_EXCEL[k]$LOCATION, pFileRDS=paste0(My_EXCEL[k]$FILES, ''), ToBrowse=F)
    Files_All[[k]] = qf
  }
  ...
  if (!ToReset)
  {
    Old_Files = CCPR_READRDS(FileFolder,FileName,  ToRestore = T)
  ...
  Final_Files = unique(rbind(Old_Files, All_Files, fill=T), by=c('location', 'files'), fromLast = T)
  Final_Files[, files := toupper(files)]
  Final_Files = Final_Files [order (files)]
  My.Kable.All(Final_Files)
  CCPR_SAVERDS(Final_Files, FileFolder,FileName, SaveOneDrive = T, ToKable = T)
  # My.Kable.All(Monitor_Funtion)
  DATACENTER_LINE_BORDER_BEGINEND('XLS_UPDATE_MONITOR_CCPR_BATCH', 'END')
  GC_SILENT()
  End.time = Sys.time()
  CATln(paste('DURATION =', Format.Number( difftime(End.time, Start.time, units='secs'), 2)))
}
...
# ==================================================================================================
REMOVE_DUPLICATED_COLUMNS = function(pData) {
  # ------------------------------------------------------------------------------------------------
  GC_SILENT()
  # pData = DATACENTER_LOADDTRDS_UDATA('download_ifrc_indfgd_history.rds')
  if (length(unique(names(pData))) != length(names(pData)))
  {
    str(pData)
    pData = pData %>% select(unique(colnames(.)))
    str(pData)
  } 
  GC_SILENT()
  return(pData)
...
# ==================================================================================================
CLEAN_FIELD_UPDATED = function(pData, ToAdd=T) {
  # ------------------------------------------------------------------------------------------------
  if (nrow(pData)>0)
  {
...
# ==================================================================================================
XLS_UPDATE_BATCH_FILES_MONITOR_CCPR = function(pMyPC) {
  # ------------------------------------------------------------------------------------------------
  #2023-09-15 17:00
  # XLS_UPDATE_MONITOR_CCPR()
  Start.time = Sys.time()
  ...
  # try(tk2dde.request("Excel", "Sheet1", "R1C1:R3C1")
  # x1 = try(tcltk2::tk2dde.request("Excel", 'MONITOR_FILES', "R1C1"), silent = T)
  x2 = try(tcltk2::tk2dde.request("Excel", My_Sheet, "R2C1"), silent = T)
  x2.time = try(tcltk2::tk2dde.request("Excel", My_Sheet, "R2C2"), silent = T)
  ...
  if (all(class(x2)!='try-error'))
  {
    ...
    x.date = try(tcltk2::tk2dde.poke("Excel", My_Sheet, "R2C1", as.character(Sys.Date()) ), silent = T)
    x.time = try(tcltk2::tk2dde.poke("Excel", My_Sheet, "R2C2", substr(as.character(Sys.time()),11,19 )), silent = T)
    x.date = try(tcltk2::tk2dde.poke("Excel", My_Sheet, "R2C10", as.character(Sys.Date()) ), silent = T)
    x.time = try(tcltk2::tk2dde.poke("Excel", My_Sheet, "R2C11", substr(as.character(Sys.time()),11,19 )), silent = T)
    CATln_Border(paste(x.date, x.time))
  ...
  # pMyPC = 'BeQ-RD3'
  # switch(pMyPC,
  #        'BeQ-RD3' = {FileName = 'CHECK_FILE_QUALITY_CCPR.rds' ; FileFolder = paste0(CCPRDrive, 'CCPR/MONITOR/')
  #        Offset_Row = 3
  #        Offset_Col = 0
  #        }
  #        ,
  #        'BeQ-RD5' = {FileName = 'CHECK_FILE_QUALITY_IFRC.rds' ; FileFolder = paste0(CCPRDrive, 'CCPR/MONITOR/')
  #        Offset_Row = 3
  #        Offset_Col = 9}
  # )
  # 
  ...
  FullPath_Local = paste0(FileFolder, FileName)
  FullPath_ODDrive = paste0(ODDrive, gsub(':','',FileFolder),FileName)
  Monitor_Funtion = CCPR_READRDS(FileFolder, FileName , ToRestore = T)
  ...
  FileName = 'CHECK_FILE_QUALITY_IFRC.rds' ; FileFolder = paste0(CCPRDrive, 'CCPR/MONITOR/')
  Offset_Row = 3
  Offset_Col = 9
  FullPath_Local = paste0(FileFolder, FileName)
  FullPath_ODDrive = paste0(ODDrive, gsub(':','',FileFolder),FileName)
...
FINAL_DOWNLOAD_VST_STKVN_SHARES_BY_CODE = function(pCode = 'VND') {
  ...
  if (all(class(content)!='try-error'))
  {
    ...
    {
      xData      = SELECT_TABLE_WITH_FIELD(tables, pField='x1', pContent="Shares outstanding", like=F)
      # xData      = try(as.data.table(tables[[6]]))
...
RELOAD_INSREF = function ( silent=T, ToForce=F) {
  ...
  if (ToForce)
  {
    if (file.exists(UData)) { 
      ...
      if (file.exists(CCPRData)) { 
        CATln('Loading ..')
        ins_ref = CCPR_READRDS(CCPRData, 'EFRC_INS_REF.rds', ToKable = T, ToRestore = T) }
    }
    assign("ins_ref", ins_ref, envir = .GlobalEnv)
  } else {
    if (exists("ins_ref")) { if (!silent) { CATln("INS_REF ALREADY LOADED") } } else { 
      # UData = CCPRData
      if (file.exists(UData)) { pFolder = UData} else {pFolder = CCPRData}
...
RUN_LOOP_BY_BROUP_AND_SAVE_V2 = function(pOption='VST_BOARD_MEMBER', Nb_Group = 50, Nb_Min = 50000, Frequency='MONTH', AddParam='') { 
  ...
  switch(pOption,
         ...
         'DOWNLOAD_YAH_STK_SIZE_PRICES'      = {
           ...
           FilePath       = paste0(CCPRData, SaveFolder, '/', FileName, '_', CurrentFileDate, '.rds')
           CATln_Border(paste("SAVE TO FILEPATH : ", FilePath))
         },
  ...
  if (nrow(List_Codes)>0)
  {
    if (file.exists(FilePath))
    {
      Data_old = CCPR_READRDS(paste0(CCPRData, SaveFolder, '/'),paste0(FileName, '_', CurrentFileDate, '.rds'))
      if (nrow(Data_old)>0) {
        Data_old = Data_old[!is.na(codesource)]
        List_Codes_ToDo = List_Codes[!codesource %in% Data_old$codesource]
        My.Kable.MaxCols(Data_old)
    ...
    if (nrow(List_Codes_ToDo)>0)
    {
      ...
      for (i in 1:Nb_TODO)
      {
        ...
        switch(pOption,
               ...
               'DOWNLOAD_YAH_STK_SIZE_PRICES'      = {
                 switch(xMethod,
                        'UNADJUST_ADJUST' = { My_Data = CHECK_CLASS(try(DOWNLOAD_YAH_INS_PRICES_UNADJ(p_nbdays_back=250000,
                                                                                                      tickers=pCode, freq.data='daily', p_saveto="", NbTry=10))) }, 
        ...
        if (i %% Nb_Group ==0 | i == Nb_TODO)
        {
          Data_All = rbindlist(Data_List, fill=T)
          Data_final = rbind(Data_old, Data_All, fill=T)
          CCPR_SAVERDS(Data_final, FileFolder = FilePath, FileToSave, SaveOneDrive = T, ToSummary = T)
...
DOWNLOAD_YAH_STK_CAPIUSD_BY_CODE = function(pCodesource='CYBQF', EchoOn=T, ToKable=T) {
  # ------------------------------------------------------------------------------------------------
  # x = DOWNLOAD_YAH_STK_CAPIUSD_BY_CODE(pCodesource='CYBQF', EchoOn=T, ToKable=T)
  # x = DOWNLOAD_YAH_STK_CAPIUSD_BY_CODE(pCodesource='X12345678', EchoOn=T, ToKable=T)
  # pCodesource='AAPL'
  xRES     = data.table()
  DATA_AVAILABLE = F
  ...
  {
    LastTrading = SYSDATETIME(9)
    xData = data.table(source='YAH', codesource=pCodesource, dataset='CAPIUSD', date=SYSDATETIME(1))
    xData = MERGE_DATASRC_CODE_BYCODESOURCE('YAH', xData)
    xData = MERGE_DATASTD_BYCODE(xData, pOption='FINAL')
    My.Kable.All(xData)
  }
...
RUN_NOSTOP_BY_PC_CCPR = function(pMyPC, ToForce=F) {
  ...
  switch(pMyPC,
         ...
         'BeQ-RD1' = {  
           ...
           # 
           if (CHECK_TIMEBETWEEN("09:30", "12:00")|CHECK_TIMEBETWEEN("15:30", "17:30") )
           {
             x = try(RUN_LOOP_STKVN(pOption='CCPR_DOWNLOAD_STB_STKVN_SHARES_BY_CODE', Nb_Group = 50, Nb_Min = 10000, AddParam='', FillBlank=T, ToForce=F)) 
             x = try(RUN_LOOP_STKVN(pOption='CCPR_DOWNLOAD_C68_STKVN_SHARES_BY_CODE', Nb_Group = 50, Nb_Min = 10000, AddParam='', FillBlank=T, ToForce=F))
           }
           # x = try(RUN_LOOP_STKVN (pOption='FINAL_DOWNLOAD_C68_STKVN_SHARES', Nb_Group = 50, Nb_Min = 5000,
           #                         AddParam='', FillBlank=T, ToForce=F) )
           # 
           # x = try(RUN_LOOP_STKVN (pOption='FINAL_DOWNLOAD_CAF_STKVN_SHARES', Nb_Group = 50, Nb_Min = 5000,
           #                         AddParam='', FillBlank=T, ToForce=F) )
           if (CHECK_TIMEBETWEEN("17:30", "21:00") |CHECK_TIMEBETWEEN("00:00", "08:00") )
           {
            try(IFRC_CCPR_DOWNLOAD_WSJ_BOARD_BY_ISO2  (pFolder = paste0(CCPRData, 'LIST/') , iso2 = 'SG'))
           }
           if (CHECK_TIMEBETWEEN("21:00", "24:00"))
           {
           x = try(RUN_LOOP_BY_BROUP_AND_SAVE_V2(pOption='DOWNLOAD_YAH_STK_SIZE_PRICES', Nb_Group = 100, Nb_Min = 20000, Frequency='MONTH',
                                                 AddParam='SMALL,UNADJUST_ADJUST'))
           }
         ...
         'BeQ-RD2' = {  
           ...
           if(CHECK_TIMEBETWEEN("09:30", "12:00")|CHECK_TIMEBETWEEN("15:30", "17:30") )
           {
           x = try(RUN_LOOP_STKVN(pOption='CCPR_DOWNLOAD_VST_STKVN_SHARES', Nb_Group = 50, Nb_Min = 10000, AddParam='', FillBlank=T, ToForce=F))
           
           x = try(RUN_LOOP_STKVN(pOption='NEW_DOWNLOAD_C68_STKVN_PERFORMANCE', Nb_Group = 50, Nb_Min = 10000, AddParam='', FillBlank=T, ToForce=F))
           z = try(RUN_LOOP_STKVN(pOption='DOWNLOAD_SSI_STKVN_PRICES', Nb_Group = 20, Nb_Min = 2000, AddParam='', FillBlank=T, ToForce=F))
           
           z = try(RUN_LOOP_STKVN(pOption='DOWNLOAD_STB_STKVN_PRICES_BY_CODE', Nb_Group = 20, Nb_Min = 2000, AddParam='', FillBlank=T, ToForce=F, ToCheckTime=16)) 
           }
           # STK_DATA = try(LOOP_DOWNLOAD_YAH_STK(Dataset='DIV', 
           #                                      Folder_List=CCPRData, File_List='LIST_YAH_STK_GBL_WCEO', 
           #                                      Folder_Out=CCPRData, Nb_Min=10000, Nb_Group=50))
           if (CHECK_TIMEBETWEEN("18:30", "23:55") |CHECK_TIMEBETWEEN("01:00", "07:55")  )
           {
             try(IFRC_CCPR_DOWNLOAD_WSJ_BOARD_BY_ISO2  (pFolder = paste0(CCPRData, 'LIST/') , ISO2 = 'SG'))
             try(IFRC_CCPR_DOWNLOAD_WSJ_BOARD_BY_ISO2  (pFolder = paste0(CCPRData, 'LIST/') , ISO2 = 'ID'))
             try(IFRC_CCPR_DOWNLOAD_WSJ_BOARD_BY_ISO2  (pFolder = paste0(CCPRData, 'LIST/') , ISO2 = 'MY'))
             try(IFRC_CCPR_DOWNLOAD_WSJ_BOARD_BY_ISO2  (pFolder = paste0(CCPRData, 'LIST/') , ISO2 = 'PH'))
         ...
         'BeQ-RD4' = {  
           if (CHECK_TIMEBETWEEN("09:30", "12:00")|CHECK_TIMEBETWEEN("15:30", "18:30") )
           {
             x = try(RUN_LOOP_STKVN(pOption='CCPR_DOWNLOAD_CAF_STKVN_SHARES_BY_CODE', Nb_Group = 50, Nb_Min = 10000, AddParam='', FillBlank=T, ToForce=F)) 
           }
           ...
           if (CHECK_TIMEBETWEEN("12:00", "15:30") )
           {
           x = try(RUN_LOOP_STKVN(pOption='NEW_DOWNLOAD_C68_STKVN_PERFORMANCE', Nb_Group = 50, Nb_Min = 10000, AddParam='', FillBlank=T, ToForce=F))
           # x = try(RUN_LOOP_BY_BROUP_AND_SAVE_V2(pOption='DOWNLOAD_YAH_STK_SIZE_PRICES', Nb_Group = 100, Nb_Min = 20000, Frequency='MONTH', 
           ...
           x = try(RUN_LOOP_STKVN(pOption='DOWNLOAD_C68_STKVN_INTRADAY', Nb_Group = 50, Nb_Min = 5000, AddParam='', FillBlank=T, ToForce=F))
           ...
           #                         AddParam='', FillBlank=T, ToForce=F) )
           if (CHECK_TIMEBETWEEN("21:00", "02:00") )
           {
           x = try(RUN_LOOP_BY_BROUP_AND_SAVE_V2(pOption='DOWNLOAD_YAH_STK_SIZE_PRICES', Nb_Group = 100, Nb_Min = 20000, Frequency='MONTH',
                                                 AddParam='MID,CLOSE'))
           }
         ...
         'BeQ-RD5' = {
           if (CHECK_TIMEBETWEEN("21:30", "02:00") )
           {
           x = try(RUN_LOOP_BY_BROUP_AND_SAVE_V2(pOption='DOWNLOAD_YAH_STK_SIZE_PRICES', Nb_Group = 100, Nb_Min = 20000, Frequency='MONTH',
                                                 AddParam='LARGE,UNADJUST_ADJUST'))
           }
           ...
           if (CHECK_TIMEBETWEEN("09:30", "12:00")|CHECK_TIMEBETWEEN("15:30", "17:30") )
           {
             x = try(RUN_LOOP_STKVN (pOption='CCPR_DOWNLOAD_C68_STKVN_SHARES', Nb_Group = 50, Nb_Min = 5000,
                                     AddParam='', FillBlank=T, ToForce=F) )
           }
           
           z = try(RUN_LOOP_STKVN(pOption='DOWNLOAD_STB_STKVN_RATIOS_VALUATION', Nb_Group = 4, Nb_Min = 3, AddParam='', FillBlank=T, ToForce=F))
           x = try(RUN_LOOP_STKVN(pOption='NEW_DOWNLOAD_CAF_STKVN_PRICES', Nb_Group = 50, Nb_Min = 10000, AddParam='', FillBlank=T, ToForce=F))
...
RUN_NOSTOP_BY_PC_CCPR_SPECIAL = function(pMyPC) {
  ...
  switch(pMyPC,
         'BeQ-RD1' = {
           try(CCPR_UPDATE_MONITOR_CCPR_FILES())
           z = try(RUN_LOOP_STKVN(pOption='DOWNLOAD_STB_STKVN_MAJOR_HOLDERS_PERCENTAGE', Nb_Group = 50, Nb_Min = 5000, AddParam='', FillBlank=T, ToForce=F))
           # x = try(RUN_LOOP_STKVN (pOption='CCPR_DOWNLOAD_VST_STKVN_SHARES', Nb_Group = 50, Nb_Min = 10000,
         ...
         'BeQ-RD5' = {
           ...
           for (i in 1:10)
           {
             x = try(RUN_LOOP_STKVN (pOption='CCPR_DOWNLOAD_CAF_STKVN_SHARES', Nb_Group = 50, Nb_Min = 10000,
                                     AddParam='', FillBlank=T, ToForce=F, ToCheckTime = 9) )
           }
         ...
         'BeQ-RD4' = {
           ...
           try(RUN_LOOP_STKVN(pOption='CCPR_DOWNLOAD_C68_STKVN_SHARES', Nb_Group = 50, Nb_Min = 10000, AddParam='', FillBlank=T, ToForce=F))
           }
           }
           # try(RUN_LOOP_STKVN(pOption='FINAL_DOWNLOAD_C68_STKVN_SHARES', Nb_Group = 50, Nb_Min = 10000, AddParam='', FillBlank=T, ToForce=F)) 
         },
         ...
         'IFRC-IREEDS' = {
           ...
           if (wday(Sys.Date()) %in% c(1,7) && (CHECK_TIMEBETWEEN("18:30", "23:30") | CHECK_TIMEBETWEEN("00:30", "07:30")))
           {
             ...
             x = try(RUN_LOOP_STKVN (pOption='CCPR_DOWNLOAD_CAF_STKVN_SHARES', Nb_Group = 20, Nb_Min = 10000,
                                     AddParam='', FillBlank=T, ToForce=F, ToCheckTime = 9) )
           ...
           {
             z = try(RUN_LOOP_STKVN(pOption='CCPR_DOWNLOAD_VST_STKVN_SHARES', Nb_Group = 50, Nb_Min = 5000, AddParam='', FillBlank=T, ToForce=F))
             z = try(RUN_LOOP_STKVN(pOption='CCPR_DOWNLOAD_STB_STKVN_SHARES', Nb_Group = 50, Nb_Min = 5000, AddParam='', FillBlank=T, ToForce=F))
             z = try(RUN_LOOP_STKVN(pOption='CCPR_DOWNLOAD_C68_STKVN_SHARES', Nb_Group = 50, Nb_Min = 5000, AddParam='', FillBlank=T, ToForce=F))
           }
...
RUN_NOSTOP_BY_PC_CCPR_QUICK = function(pMyPC){
  ...
  switch(pMyPC,
         ...
         'IFRC-103'     = {  
           try(CCPR_CHECK_MYPC(pSleep=round(runif(1,30,60))))
           try(CHECK_FILE_QUALITY_IFRC())
         },
         ...
         },
         {
           ...
           # try(UPDATE_UPLOAD_CCPR_MARKET_LAST(UData=CCPRData, UList= paste0(CCPRData, 'YAH/LIST/','LIST_YAH_MARKET_LAST.txt'), 
           #                                    pSource = 'YAH',  xPeriod = '5m', toReset = F))
           IFRC_SLEEP(runif(1,1,10))
           # FINAL_DOWNLOAD_CAF_STKVN_SHARES_BY_CODE(pCode = 'AAV')
         }
  )
...
CCPR_WRITE_SUMMARY_CURRENT = function(pData=data.table(), pFileName, ToExclude=T, ToPrompt=F, ToRemoveCodeNA=T) {
  ...
  if (all(class(x.rds)!='try-error') && nrow(x.rds)>0)
  {
    ...
    {  
      my.summary = x.rds[date<=Sys.Date(),.(start=date[1], date=date[.N], nb=.N, datelast=date[.N], last=close[.N], 
                                            nbdays=as.numeric(NA), source=source[.N], name="", type="", 
                                            codesource=codesource[.N]), by='code']
    } else {
      my.summary = my.data[,.( 
        start=date[1], date=date[.N], nb=.N, datelast=date[.N], 
        last=close[.N], nbdays=as.numeric(NA), source=source[.N],
...
CCPR_CHECK_MYPC = function(pSleep = 10) {
  ...
  try(SAVERDS_WITH_ONEDRIVE(pData=Data_MYPC, FileFolder=paste0(CCPRData, 'MONITOR/'), FileName=FileName , SaveOneDrive=xSaveOneDrive))
  IFRC_SLEEP(pSleep)
}
...
# ==================================================================================================
RUN_LOOP_STKVN = function(pOption='IFRC_DOWNLOAD_HSX_STKVN_REF', Nb_Group = 50, Nb_Min = 50000, AddParam='', FillBlank=T, ToForce=F, ToCheckTime=17) { 
  # ------------------------------------------------------------------------------------------------
  ...
  switch(pOption,
         'CCPR_DOWNLOAD_CAF_STKVN_SHARES_BY_CODE' = {
           FileName    =  paste0('CCPR_DOWNLOAD_CAF_STKVN_SHARES_', gsub('-','', LastTrading))
           # FileNameDay =  paste0('DOWNLOAD_STB_STKVN_MAJOR_HOLDERS_PERCENTAGE_DAY')
         },
         'CCPR_DOWNLOAD_STB_STKVN_SHARES_BY_CODE' = {
           FileName    =  paste0('CCPR_DOWNLOAD_STB_STKVN_SHARES_', gsub('-','', LastTrading))
           # FileNameDay =  paste0('DOWNLOAD_STB_STKVN_MAJOR_HOLDERS_PERCENTAGE_DAY')
         },
         ...
         'DOWNLOAD_C68_STKVN_INTRADAY' = {
           ...
           FileName =  paste0('DOWNLOAD_C68_STKVN_INTRADAY_', gsub('-','', LastTrading))
         }, 
         ...
         'CCPR_DOWNLOAD_C68_STKVN_SHARES' = {
           FileName =  paste0('CCPR_DOWNLOAD_C68_STKVN_SHARES_', gsub('-','', LastTrading))
           # FileNameDay =  paste0('DOWNLOAD_C68_STKVN_SHARES_DAY')
         }, 
         'CCPR_DOWNLOAD_VST_STKVN_SHARES' = {
           FileName =  paste0('CCPR_DOWNLOAD_VST_STKVN_SHARES_', gsub('-','', LastTrading))
         ...
         'DOWNLOAD_STKVN_PRICES_FOR_ALL' = { 
           ...
           FileName = paste0('CCPR_STKVN_PRICES_FOR_ALL_', gsub('-','', LastTrading))
         },
         'DOWNLOAD_STKVN_OWNERSHIP_FOR_ALL' = { 
           FileName = paste0('CCPR_STKVN_OWNERSHIP_FOR_ALL_', gsub('-','', LastTrading))
         },
         'DOWNLOAD_STKVN_PE_FOR_ALL' = {
           FileName = paste0('CCPR_STKVN_PE_FOR_ALL_', gsub('-','', LastTrading))
         },
         'DOWNLOAD_STKVN_SHAREHOLDER_FOR_ALL' = {
           FileName = paste0('CCPR_STKVN_SHAREHOLDER_FOR_ALL_', gsub('-','', LastTrading))
         },
         'DOWNLOAD_CAF_STKVN_ANNUAL_RATIOS_BY_CODE' = {
           FileName = paste0('DOWNLOAD_CAF_STKVN_ANNUAL_RATIOS_', gsub('-','', LastTrading))
         },
         'DOWNLOAD_STKVN_PRICES'   = {
           FileName = paste0('CCPR_STKVN_PRICES_ALL_', gsub('-','', LastTrading))
         },
         'DOWNLOAD_STB_STKVN_RATIOS_VALUATION'  = {
           FileName = paste0('CCPR_STKVN_RATIOS_VALUATION_', gsub('-','', LastTrading))
         }, 
         'NEW_DOWNLOAD_C68_STKVN_PERFORMANCE'  = {
           FileName = paste0('CCPR_STKVN_C68_PERFORMANCE_', gsub('-','', LastTrading))
         }, 
         'FINAL_DOWNLOAD_C68_STKVN_INTRADAY_BY_CODE'  = {
           FileName = paste0('CCPR_STKVN_C68_INTRADAY_BY_CODE_', gsub('-','', LastTrading))
         }, 
         'NEW_DOWNLOAD_CAF_STKVN_TRADE_BY_CODE'  = {
           FileName = paste0('NEW_DOWNLOAD_CAF_STKVN_TRADE_', gsub('-','', LastTrading))
         }, 
         'NEW_DOWNLOAD_CAF_STKVN_FOREIGN_BY_CODE'  = {
           FileName = paste0('NEW_DOWNLOAD_CAF_STKVN_FOREIGN_', gsub('-','', LastTrading))
         }, 
         'DOWNLOAD_HNX_STKVN_SHARES_BY_CODE'  = {
           FileName    =  paste0('DOWNLOAD_HNX_STKVN_SHARES_', gsub('-','', LastTrading))
         },
         'IFRC_DOWNLOAD_HSX_STKVN_REF'  = {
           FileName    =  paste0('IFRC_DOWNLOAD_HSX_STKVN_REF_', gsub('-','', LastTrading))
         },
         'CCPR_DOWNLOAD_HSX_STKVN_SHARES'  = {
           FileName    =  paste0('CCPR_DOWNLOAD_HSX_STKVN_SHARES_', gsub('-','', LastTrading))
         }
  ...
  if (nrow(List_Codes)>0)
  {
    if (file.exists(FilePath))
    {
      Data_old = CCPR_READRDS(CCPRData, paste0(FileName, '.rds')) 
      if (nrow(Data_old)>0) {
        Data_old = Data_old[!is.na(codesource)]
        List_Codes_ToDo = List_Codes[!codesource %in% Data_old$codesource ]
        My.Kable.MaxCols(Data_old)
    ...
    if (nrow(List_Codes_ToDo)>0)
    {
      ...
      for (i in 1:Nb_TODO)
      {
        ...
        switch(pOption,
               ...
               'CCPR_DOWNLOAD_CAF_STKVN_SHARES_BY_CODE' = {
                 My_Data = try(CCPR_DOWNLOAD_CAF_STKVN_SHARES_BY_CODE (pCode = pCode))
               ...
               'CCPR_DOWNLOAD_STB_STKVN_SHARES_BY_CODE' = {
                 My_Data = try(CCPR_DOWNLOAD_STB_STKVN_SHARES_BY_CODE (pCode = pCode))
               ...
               'CCPR_DOWNLOAD_HSX_STKVN_SHARES'      = {
                 ...
                 My_Data = try(IFRC_DOWNLOAD_HSX_STKVN_REF(pFolder=CCPRData, ToIntegrateHistory=T))
                 # if (FillBlank && nrow(My_Data)==0)
                 # {
                 #   # LastTrading = CCPR_LAST_TRADING_DAY_VN(hour(Sys.time()), ToPrompt = F)
                 #   My_Data = data.table(source='HSX', codesource=pCode, code=paste0('STKVN', pCode), date=LastTrading)
                 #   My.Kable.All(My_Data)
                 # }
               },
               ...
               'CCPR_DOWNLOAD_C68_STKVN_SHARES'      = {
                 # pCode = 'X18'
                 My_Data = try(CCPR_DOWNLOAD_C68_STKVN_SHARES_BY_CODE(pCode = pCode))
                 if (FillBlank && nrow(My_Data)==0)
                 {
                   # LastTrading = CCPR_LAST_TRADING_DAY_VN(hour(Sys.time()), ToPrompt = F)
                   My_Data = data.table(source='C68', codesource=pCode, code=paste0('STKVN', pCode), date=LastTrading)
               ...
               'CCPR_DOWNLOAD_VST_STKVN_SHARES'      = {
                 # pCode = 'X18'
                 My_Data = try(CCPR_DOWNLOAD_VST_STKVN_SHARES_BY_CODE(pCode = pCode))
                 if (FillBlank && is.null(nrow(My_Data)) )
                 {
                   # LastTrading = CCPR_LAST_TRADING_DAY_VN(hour(Sys.time()), ToPrompt = F)
        ...
        if (i %% Nb_Group ==0 | i == Nb_TODO)
        {
          ...
          CCPR_SAVERDS(Data_final,CCPRData, FileToSave, ToSummary = T, SaveOneDrive = T)
          # DAY :
          ...
          CCPR_SAVERDS(data_all_final,CCPRData,FileAll , ToSummary = T, SaveOneDrive = T)
          ...
          CCPR_SAVERDS(data_all_final_history,CCPRData,FileNameHistory , ToSummary = T, SaveOneDrive = T)
          
          CATln(paste('SAVED : ', FileNameHistory, '-', Format.Number(nrow(Data_final),0),'records'))
          Sys.sleep(2)
        }
        End.Time = Sys.time()
        Total_Time = as.numeric(difftime(End.Time, Start.Time, units = 'secs')) + Total_Time
...
CCPR_QUICK_INTEGRATION_DAY_FINAL = function(pFolder=UData, pData=data.table(), 
  ...
  # pData   = xFINAL
  # File_To = 'download_exc_test_history.rds'
  ...
  if (ToForce || TODO)
  {
    ...
    if (nrow(pData)==0 && file.exists(paste0(pFolder, File_Fr)))
    {
      Data_Fr = try(readRDS(paste0(pFolder, File_Fr)))
    ...
    if (nrow(Data_Fr)>0 && file.exists(paste0(pFolder, File_To)))
    {
      Data_To = try(readRDS(paste0(pFolder, File_To)))
    ...
    if (nrow(Data_Final)>0)
    {
      ...
      Data_Final = unique(MERGE_DATASTD_BYCODE(Data_Final, pOption='FINAL')[order(code, date)], by=c('code', 'date'))
      My.Kable.MaxCols(Data_Final)
      SAVERDS_WITH_ONEDRIVE(Data_Final, pFolder, File_To, SaveOneDrive = SaveOneDrive, ToSummary=T)
    }
  }
}
...
# ==================================================================================================
DOWNLOAD_YAH_STK_SHARES_BY_CODE = function(pCode='AAPL', EchoOn=F, ToKable=T)  {
  # ------------------------------------------------------------------------------------------------
  # x = DOWNLOAD_YAH_STK_SHARES_BY_CODE(pCode='IBM', EchoOn=F, ToKable=T)
  # pCode='AAPL'
  # pCodesource
  xRES_SHARESOUT = data.table()
  xRES_SHARESFF  = data.table()
  xRES     = data.table()
  ...
  if (ToContinu)
  {
    xSHARES    = as.data.table(tables[[3]])
    if (EchoOn) { My.Kable.All(xSHARES) }
    xBODY = xSHARES[grepl("^Shares Outstanding",X1)]
    
    if (nrow(xBODY)==1) 
    { 
      temp  = trimws(xBODY$X2)
      xunit = substr(temp,nchar(temp), nchar(temp))
      switch(xunit,
             'T' = {capi_unit = 1000000000000},
             'B' = {capi_unit = 1000000000},
      ...
      if (EchoOn) { print(paste("IN : ", temp, "/", xunit))}
      close_unit = as.numeric(gsub(',','', substr(temp,1,nchar(temp)-1)))
      xRES_SHARESOUT = data.table(source='YAH', dataset='SHARES OUTSTANDING', codesource=pCode, date=SYSDATETIME(23), 
                                  close_unit = as.numeric(gsub(',','', substr(temp,1,nchar(temp)-1))),
                                  unit = capi_unit)
      xRES_SHARESOUT[, close:=close_unit*unit]
    }
    
    xBODY = xSHARES[grepl("^Float",X1)]
    if (nrow(xBODY)==1) 
    { 
      temp  = trimws(xBODY$X2)
      xunit = substr(temp,nchar(temp), nchar(temp))
      ...
      if (EchoOn) { print(paste("IN : ", temp, "/", xunit))}
      close_unit = as.numeric(gsub(',','', substr(temp,1,nchar(temp)-1)))
      xRES_SHARESFF = data.table(source='YAH', dataset='SHARES FLOAT', codesource=pCode, date=SYSDATETIME(23), 
                                 close_unit = as.numeric(gsub(',','', substr(temp,1,nchar(temp)-1))),
                                 unit = capi_unit)
      xRES_SHARESFF[, close:=close_unit*unit]
      # str(xRES_SHARESFF)
    }
    
    xRES_ALL = rbind(xRES_SHARESOUT,xRES_SHARESFF, fill=T)
    xRES_ALL = MERGE_DATASRC_CODE_BYCODESOURCE('YAH', xRES_ALL)
    xRES_ALL = MERGE_DATASTD_BYCODE(xRES_ALL, pOption='FINAL')
    if (ToKable) { My.Kable.All(xRES_ALL[, -c('type', 'fcat', 'scat', 'continent')]) }
  }
  GC_SILENT()
  return(xRES_ALL)
}
...
# ==================================================================================================
CCPR_SMART_INTEGRATION_DAY = function(Folder_Fr=UData, Folder_To=UData, pData=data.table(), 
                                      File_Fr='download_caf_stkvn_shares_history.rds', File_To='download_caf_stkvn_shares_day.rds',
  ...
  # Folder_Fr=UData; Folder_To=UData; pData=data.table(); 
  # File_Fr='download_rts_stk_history.rds'; File_To='efrc_stk_history.rds';
  # pCondition='type=="STK"'; IncludedOnly=F; CheckDateSummary=F; MaxDateOnly=F; SaveOneDrive=T; Merge_STD=F; ToForce=F
  
  # Folder_Fr=paste0(CCPRData, 'YAH/YYYYMM/'); Folder_To=paste0(CCPRData, ''); pData=data.table(); 
  # File_Fr='DOWNLOAD_YAH_STK_SIZE_MID_202309.rds'; File_To='DOWNLOAD_YAH_STK_SIZE_MID_HISTORY.rds';
  # pCondition=''; IncludedOnly=F; CheckDateSummary=F; MaxDateOnly=F; NbDays=0; SaveOneDrive=F; Merge_STD=F; ToForce=F
  ...
  DATACENTER_LINE_BORDER(paste('CCPR_QUICK_INTEGRATION_DAY_FINAL : ', File_To))
  TODO = T
  CATln(CCPR_SUMMARY_DATE_RDS(paste0(Folder_Fr, File_Fr)))
  CATln(CCPR_SUMMARY_DATE_RDS(paste0(Folder_To, File_To)))
  Data_Final = data.table()
  ...
  if (CheckDateSummary && nchar(File_To)>0 & nchar(File_Fr)>0)
  {
    CheckDateSummary = T
    CATln(paste("CHECK DATE SUMMARY :", File_Fr, '/', File_To), top_border=T, p_border = T)
    TODO             = (CCPR_SUMMARY_DATE_RDS(paste0(Folder_To, File_To)) < CCPR_SUMMARY_DATE_RDS(paste0(Folder_Fr, File_Fr)))
  } else {
    if (!file.exists(paste0(Folder_To, File_To)))
    {
      TODO = T
    }
  }
  ...
  if (ToForce || TODO)
  {
    if (nrow(pData)>0)
    {
      ...
      if ('indhome' %in% names(Data_Fr)) { Data_Fr[, indhome:=as.numeric(indhome)]}
      if ('stkhome' %in% names(Data_Fr)) { Data_Fr[, stkhome:=as.numeric(stkhome)]}
      if ('home' %in% names(Data_Fr)) { Data_Fr[, home:=as.numeric(home)]}
      Data_Fr = unique(MERGE_DATASTD_BYCODE(Data_Fr)[order(code, date)], by=c('code', 'date'))
      
    } 
    
    if (nrow(pData)==0 && file.exists(paste0(Folder_Fr, File_Fr)))
    {
      ...
      if (all(class(Data_Fr)!='try-error') && nrow(Data_Fr)>0)
      {
        ...
        Data_Fr = Data_Fr[!is.na(code) & !is.na(date)]
      }
    }
    
    if (nrow(Data_Fr)>0)
    {
      My.Kable.Min(Data_Fr[order(date, code)])
      My.Kable.Min(Data_Fr[!is.na(code) & !is.na(date)][order(date, code)])
    }
    
    # DATA_FROM (END) ..............................................................................
    # str(Data_Fr)
    
    if (nrow(Data_Fr)>0 && file.exists(paste0(Folder_To, File_To)))
    {
      Data_To = try(CCPR_READRDS(Folder_To, File_To, ToKable = T, ToRestore = T))
      N0 = nrow(Data_To)
      if (all(class(Data_To)!='try-error') && nrow(Data_To)>0)
      {
        Data_To = unique(MERGE_DATASTD_BYCODE(Data_To)[order(code, date)], by=c('code', 'date'))
        
        # str(Data_To)
        # Data_To = CLEAN_FIELDS_DATATABLE(Data_To)
        if ('indhome' %in% names(Data_To)) { Data_To[, indhome:=as.numeric(indhome)]}
        if ('stkhome' %in% names(Data_To)) { Data_To[, stkhome:=as.numeric(stkhome)]}
        if ('home' %in% names(Data_To)) { Data_To[, home:=as.numeric(home)]}
        ...
        # str(Data_To)
        Data_Final = unique(rbind(Data_To, Data_Fr, fill=T), by=c('code', 'date'), FromLast=T)[order(code, date)]
      } else {
        Data_Final = Data_Fr
      }
    } else {
    ...
    My.Kable.Min(Data_Final[order(date, code)])
    My.Kable.Min(Data_Final[!is.na(code) & !is.na(date)][order(date, code)])
    ...
    if (nrow(Data_Final)>0)
    {
      ...
      if (NbDays>0)
      {
        Data_Final[, datemax := max(date, by='code')]
        Data_Final = Data_Final[as.numeric(datemax)-as.numeric(date)<=(NbDays+1)]
        Data_Final$datemax = NULL
      }
      ...
      CCPR_SAVERDS(Data_Final, Folder_To, File_To, SaveOneDrive = SaveOneDrive, ToSummary=T)
      CATln(paste('From = ', Format.Number(N0,0), ', To = ', Format.Number(N1,0), ' - NEW RECORDS =', Format.Number((N1-N0),0)))
      CATln_Border('REPORT')
      IFRC_DATA_QUICK_REPORT(Data_Final)
    }
  }
}
